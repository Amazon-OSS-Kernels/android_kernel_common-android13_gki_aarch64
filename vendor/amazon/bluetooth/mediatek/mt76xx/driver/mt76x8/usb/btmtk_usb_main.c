...
const struct proc_ops BT_chip_reset_delay_proc_fops = {
	.proc_open = btmtk_chip_reset_delay_proc_open,
	.proc_read = seq_read,
	.proc_write = btmtk_chip_reset_delay_proc_write,
	.proc_release = single_release,
};
#endif
...
static inline unsigned long __nocfi btmtk_kallsyms_lookup_name(const char *name)
{
...
static void btmtk_chip_rst_disc_timo_func(struct timer_list *timer)
{
	int ret;
	...
	do {
		...
		set_pin_state_func_ptr set_pin_state_func =
			(set_pin_state_func_ptr) btmtk_kallsyms_lookup_name(func_name);
	...
	do {
		...
		usb_logical_disconnect_ptr usb_logical_disconnect_func =
			(usb_logical_disconnect_ptr) btmtk_kallsyms_lookup_name(func_name);
...
static void btmtk_fw_dump_timo_func(struct timer_list *timer)
{
	BTUSB_INFO("%s", __func__);
	btmtk_usb_toggle_rst_pin();
}

static void btmtk_chip_reset_timo_func(struct timer_list *timer)
{
	BTUSB_INFO("%s", __func__);
...
static void btmtk_add_timer(struct timer_list *timer, void (*fun)(struct timer_list*), u16 sec, void *data)
{
	if (timer == NULL || sec == 0) {
...
void btmtk_usb_toggle_rst_pin(void)
{
	...
	do {
		...
		toggle_pin_func_ptr toggle_pin_func =
			(toggle_pin_func_ptr) btmtk_kallsyms_lookup_name(func_name);
...
static int btmtk_usb_BT_init(void)
{
	...
	do {
		...
		register_early_suspend_func = (register_early_suspend)
			btmtk_kallsyms_lookup_name(register_early_suspend_func_name);
		register_late_resume_func = (register_late_resume)
			btmtk_kallsyms_lookup_name(register_late_resume_func_name);
...
load_patch_protect:
	...
	do {
		...
		pdwndFunc = (pdwnc_func) btmtk_kallsyms_lookup_name(pdwnc_func_name);
		if (pdwndFunc) {
