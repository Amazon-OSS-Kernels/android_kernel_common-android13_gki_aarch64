...
#if MTK_DTV_GPU_MFDEC
#include <mali_kbase_mtk_dtv_gdec.h>
#endif
...
#ifdef MTK_DTV_GPU_MFDEC
int kbase_backend_nr_mfdec_atoms_submitted(struct kbase_device *kbdev, int js)
{
	int nr = 0;
	int i;
	...
	for (i = 0; i < SLOT_RB_SIZE; i++) {
		...
		if (katom &&
				...
				(katom->atom_flags & KBASE_KATOM_FLAG_USE_MFDEC))
			nr++;
	}
...
#if MTK_DTV_GPU_MFDEC
extern void invalidate_mfdec_cache(struct kbase_device* kbdev);
#endif
...
#if MTK_DTV_GPU_MFDEC
	if (katom->atom_flags & KBASE_KATOM_FLAG_USE_MFDEC)
		if (kbase_backend_nr_mfdec_atoms_submitted(kbdev, js) <= 0) {
			//invalidate_mfdec_cache(kbdev);
			mfdec_katom_unbind_slots(kbdev, katom);
		}
#endif
