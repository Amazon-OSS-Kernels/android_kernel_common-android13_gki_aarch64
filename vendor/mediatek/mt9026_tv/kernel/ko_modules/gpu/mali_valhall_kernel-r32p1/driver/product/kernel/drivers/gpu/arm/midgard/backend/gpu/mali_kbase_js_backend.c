...
#include "mali_kbase_platform_mtk_dtv.h"
#endif
...
#if defined(CONFIG_MALI_DEBUG) && defined(MTK_DTV_GPU_MFDEC)
					print_debug_status(kbdev);
#endif
					kbase_job_slot_hardstop(atom->kctx, s,
									atom);
/* MTK_DTV Patch Begin */
#if 0//defined(CONFIG_MALI_DEBUG)
					dev_warn(kbdev->dev, "*****Stopping CPU!!*****");
					kbdev->stop_cpu(kbdev);
#endif
/* MTK_DTV Patch End */
...
/* MTK_DTV Patch */
#if 0//defined(CONFIG_MALI_DEBUG)
		dev_err(kbdev->dev, "JS: Job has been on the GPU for too long (JS_RESET_TICKS_SS/DUMPING timeout hit). *NOT* issuing GPU soft-reset to resolve.");
#else
