...
static u32 dma_te_buf_fill(struct dma_buf *dma_buf, unsigned int value)
{
	...
	if (ret)
		goto no_cpu_access;
...
#if defined(MTK_DTV) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 61)) && (LINUX_VERSION_CODE < KERNEL_VERSION(5, 6, 0))
			/* Android ion in kernel 5.4 has removed the kmap in between the dma_buf cpu access.
			 * Use vmap here for cpu access as a workaround.
			 * NOTE: This call may fail due to lack of virtual mapping address space.
			 */
			void *addr = NULL;
			...
			memset(addr, value, dma_buf->size);
			dma_buf_vunmap(dma_buf, addr);
#else
...
#endif /* defined(MTK_DTV) && (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 61)) && (LINUX_VERSION_CODE < KERNEL_VERSION(5, 6, 0)) */
