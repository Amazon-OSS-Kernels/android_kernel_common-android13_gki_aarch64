	...
	const unsigned char *key, unsigned int key_len)
{
	...
	if (register_hash(&sha256_desc) < 0) {
		dprintf(CRITICAL, "%s: Register hash failed\n", __FUNCTION__);
		ret = -ERR_UNLOCK_HASH_FAIL;
		goto error;
	}
	int sha256_idx = find_hash("sha256");

	if (sha256_idx < 0) {
		dprintf(CRITICAL, "%s: Hash algorithm is not found\n", __FUNCTION__);
		ret = -ERR_UNLOCK_HASH_FAIL;
		goto error;
	}

	if (hash_memory(sha256_idx, data, data_len, digest, &digest_len) != CRYPT_OK) {
		dprintf(CRITICAL, "%s: Hash memory failed\n", __FUNCTION__);
		ret = -ERR_UNLOCK_HASH_FAIL;
		goto error;
	}
