...
void cmd_oem_relock(const char *arg, void *data, unsigned sz)
{
    ...
    fastboot_okay("");
    DSTSBLT_CHECKPOINT(DSTSBLT_CP_RELOCK_DONE);
}
...
#if defined(UFBL_FEATURE_SECURE_BOOT)
    rc |= (AMZN_ENGINEERING_DEVICE == amzn_target_device_type());
#endif
#if defined(UFBL_FEATURE_TEMP_UNLOCK)
...
void amzn_flash_unlock_signature(const unsigned char *sig,
    ...
    DSTSBLT_CHECKPOINT(DSTSBLT_CP_UNLOCKFLASH_START);

    if (sig_len < SIGNED_UNLOCK_CODE_LEN) {
        DSTSBLT_SECTRIG(DSTSBLT_SECTRIG_FLASHUNLOCK_FAILLEN, &sig_len, sizeof(sig_len));
        fastboot_fail("signature length error, wrong signature file? do nothing!");
        return;
    }
    if (amzn_verify_unlock(sig, SIGNED_UNLOCK_CODE_LEN)) {
        DSTSBLT_SECTRIG(DSTSBLT_SECTRIG_FLASHUNLOCK_FAILAUTH, NULL, 0);
    ...
    amzn_check_unlock_status();
    fastboot_okay("");
    DSTSBLT_CHECKPOINT(DSTSBLT_CP_UNLOCKFLASH_END);
}
...
int amzn_flash_onetime_unlock_code_or_cert_maybe(const unsigned char *arg,
    ...
    if (!strcmp(arg, "otucode")) {
        ...
        // unlock status might changed, recheck;
        dprintf(CRITICAL, "amzn_flash_onetime_unlock...2..() called\n");
        amzn_check_unlock_status();
        return 1;
    }
    return 0;
