...
{
	// Get public key based on Yubikey public key tag in RPMB for AMMO products
	return amzn_yk_get_root_pub_key((const uint8_t **)key, key_len);
}
#else
...
int amzn_get_temp_unlock_idme_cert(temp_unlock_cert_t **cert, unsigned int *len)
{
	...
	if (!amzn_get_temp_unlock_idme_data("t_unlock_cert",
		...
		DSTSBLT_CHECKPOINT(DSTSBLT_CP_TUCERT_PRESENT);
		*cert = tu_cert;
		ret = 0;
	}
done:
...
int amzn_get_temp_unlock_idme_code(unsigned char **sig, unsigned int *len)
{
	...
	if (!amzn_get_temp_unlock_idme_data("t_unlock_code", tu_code, len)) {
		DSTSBLT_CHECKPOINT(DSTSBLT_CP_TUCODE_PRESENT);
		*sig = tu_code;
		ret = 0;
	}
done:
...
int amzn_target_is_temp_unlocked(void)
{
	...
	verify_fail = amzn_verify_temp_unlock_code(sig, RSA_2048_SIG_LEN);
	if (!verify_fail) {
		is_unlocked = 1;
	} else {
		DSTSBLT_SECTRIG(DSTSBLT_SECTRIG_TUNLOCK_FAILAUTH, &verify_fail, sizeof(verify_fail));
	}

	DSTSBLT_CHECKPOINT(DSTSBLT_CP_TU_CHECK_DONE);
done:
	return is_unlocked;
}
#endif
