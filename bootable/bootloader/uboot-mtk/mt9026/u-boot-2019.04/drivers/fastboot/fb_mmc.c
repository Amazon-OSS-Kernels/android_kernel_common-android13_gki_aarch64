...
#define RAW_PARTITION_NAME "fastboot_raw_partition_\0"
#define RAW_PARTITION_NAME_LEN  24
#define UENV_PARTITION_NAME "uenv"
#define UENV_PARTITION_NAME_LEN  4
#define UENV_KEEP_MACADDR  true
#define MAC_BUF_SIZE 32
				     ...
				     struct disk_partition *info)
{
	...
	char env_desc_name[RAW_PARTITION_NAME_LEN + PART_NAME_LEN + 1];
	char *raw_part_desc;
	const char *argv[2];
	const char **parg = argv;

	/* check for raw partition descriptor */
	strncpy(env_desc_name, RAW_PARTITION_NAME, RAW_PARTITION_NAME_LEN);
			    ...
			    struct disk_partition *info)
{
    ...
    if(name==NULL || strlen(name)>=PART_NAME_LEN)
    {
        printf("partition name is null or strlen>=PART_NAME_LEN !!!\n");
        return -1;
    }
		...
		lbaint_t blk, lbaint_t blkcnt, const void *buffer)
{
    ...
    static unsigned long* bounce_buf;
	lbaint_t ret_blk, blk_w, r_blk, start;

	//check buffer alignment
	if (((uintptr_t)buffer & 0xFF) != 0 ) {
		//check bounce buffer
		if (!bounce_buf) {
			bounce_buf = (unsigned long*)memalign(ARCH_DMA_MINALIGN, BOUNCE_BUFFER_SIZE);
			if (!bounce_buf) {
				printf("\n\nallocate bounce buffer fail \n");
				return -1;
			}
			printf("\n\nallocate bounce buffer size is 0x%x\n", BOUNCE_BUFFER_SIZE);
		}
		r_blk = blkcnt;
		ret_blk = 0;
		start = blk;
		while (blkcnt) {
			...
			r_blk = fb_mmc_blk_write(dev_desc, start, blk_w, bounce_buf);
			ret_blk += r_blk;
			if (r_blk != blk_w)
				return ret_blk;
			start += blk_w;
				...
				char *response)
{
    ...
    /* check info.blksz <=0 retrun error */
    if(info.blksz<=0)
    {
        pr_err("unable to get block size \n");
        fastboot_fail("unable to get block size", response);
        return -1;
    }
			      ...
			      u32 download_bytes, char *response)
{
    ...
    if (!info.name[0] && (fastboot_mmc_get_part_info(cmd, &dev_desc, &info, response) < 0)) {
        return;
    }
...
#if UENV_KEEP_MACADDR
    char buffer[MAC_BUF_SIZE] = "\0";
    char *s = NULL;
    int len = -1;
    if (strncmp(cmd, UENV_PARTITION_NAME, UENV_PARTITION_NAME_LEN) == 0){
        s = env_get("macaddr");
    }
    if (s != NULL) {
        memset(buffer, 0x00, MAC_BUF_SIZE);
        len = snprintf(buffer, MAC_BUF_SIZE-1, "%s", s);
        if ( (len>0) && (env_load()==0) ) {
            if ( (env_set("macaddr", buffer)==0) && (env_save()==0) )
                printf("%s: restored macaddr %s after uenv flashing\n", __func__, buffer);
        }
    }
    if (strncmp(cmd, UENV_PARTITION_NAME, UENV_PARTITION_NAME_LEN) == 0){
        s = env_get("ethaddr");
    ...
    if (s != NULL) {
        ...
        len = snprintf(buffer, MAC_BUF_SIZE-1, "%s", s);
        if ( (len>0) && (env_load()==0) ) {
            if ( (env_set("ethaddr", buffer)==0) && (env_save()==0) )
                printf("%s: restored macaddr %s after uenv flashing\n", __func__, buffer);
        }
    }
#endif
 ...
 * @response: Pointer to fastboot response buffer
 */
#define FB_CLEAN_BUFFER_SIZE   0x1000UL //write 4k 0 to partition
void fastboot_mmc_erase(const char *cmd, char *response)
{
	struct blk_desc *dev_desc;
	struct disk_partition info= {0};
	lbaint_t blks, blks_start, grp_size;
	lbaint_t blks_size;
	struct mmc *mmc = find_mmc_device(CONFIG_FASTBOOT_FLASH_MMC_DEV);
    char clean_buffer[FB_CLEAN_BUFFER_SIZE]={0};
    ...
    if(mmc==NULL)
    {
        fastboot_fail("device mmc not found", response);
        return;
    }
    ...
    blks = blk_dwrite(dev_desc, blks_start, (FB_CLEAN_BUFFER_SIZE/dev_desc->blksz),clean_buffer);//for ENABLE FDE ISSUE
    printf("erase write 0x00 blks=0x%lx \n",blks);
