...
smp_spin_lock_t blk_spin_lock = INIT_SPIN_LOCK;
#endif
			...
			lbaint_t blkcnt, void *buffer)
{
	...
	smp_spin_lock_save(&blk_spin_lock, irq_flag);
#endif
	if (blkcache_read(block_dev->if_type, block_dev->devnum,
			  start, blkcnt, block_dev->blksz, buffer)) {
#if defined(CONFIG_MULTICORES_PLATFORM)
		smp_spin_unlock_restore(&blk_spin_lock, irq_flag);
#endif
		return blkcnt;
	}
#if defined(CONFIG_MULTICORES_PLATFORM)
