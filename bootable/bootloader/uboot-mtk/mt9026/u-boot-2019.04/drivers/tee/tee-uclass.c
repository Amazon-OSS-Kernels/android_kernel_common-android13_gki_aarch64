...
static mtk_shm_pool shm_pool = {0};
static void *mtk_alloc_simple(size_t bytes, int align)
{
	ulong addr, new_ptr;
	void *ptr;

	addr = ALIGN(shm_pool.malloc_base + shm_pool.malloc_ptr, align);
	new_ptr = addr + bytes - shm_pool.malloc_base;
	log_debug("size=%zx, ptr=%lx, limit=%lx: ", bytes, new_ptr,
		  shm_pool.malloc_limit);
	if (new_ptr > shm_pool.malloc_limit) {
		UBOOT_ERROR("alloc space exhausted, alloc_size=%zx, used_size=%lx\n", bytes, new_ptr);
		return NULL;
	}

	ptr = map_sysmem(addr, bytes);
	shm_pool.malloc_ptr = ALIGN(new_ptr, sizeof(new_ptr));
...
void *mtk_malloc_simple(size_t bytes)
{
	void *ptr;

	ptr = mtk_alloc_simple(bytes, 1);
	if (!ptr)
		return ptr;
...
void *mtk_memalign_simple(size_t align, size_t bytes)
{
	void *ptr;

	ptr = mtk_alloc_simple(bytes, align);
	if (!ptr)
		return ptr;
...
void mtk_malloc_simple_info(void)
{
	UBOOT_INFO("tee shm: 0x%lx limit: 0x%lx\n", shm_pool.malloc_base,
		shm_pool.malloc_limit);
}
#endif
				...
				struct tee_version_data *vers)
{
	...
	int ret = parse_dt("/mmap_info/E_MMAP_ID_OPTEE_MEM_SHM", mmap_dt_parser, (void*)&mmap, "reg");
	if (ret < 0) {
		UBOOT_ERROR("Warning: OPTEE_MEM_SHM information parse error in DTS\n");
	} else {
		shm_pool.malloc_base = phyical_addr_to_virtual_addr((mmap.address - MIU0_BUS_BASE));
		shm_pool.malloc_limit = mmap.size;
		mtk_malloc_simple_info();
        }
	if(shm_pool.malloc_base == 0xffffffffffffffff) {
		return NULL;
	}
#endif
