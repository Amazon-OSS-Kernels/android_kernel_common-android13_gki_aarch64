...
int dm_mmc_select_drive_strength(struct udevice *dev, int mode)
{
	struct dm_mmc_ops *ops = mmc_get_ops(dev);

	if (ops->select_drive_strength)
		return ops->select_drive_strength(dev, mode);
...
{
	return dm_mmc_select_drive_strength(mmc->dev, mode);
}
