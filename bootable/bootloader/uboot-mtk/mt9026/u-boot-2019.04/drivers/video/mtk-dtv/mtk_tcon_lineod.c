	...
	while (u8TabIndex < u8TableNum)
    	{
 		if (bChannelSeparate)
		{
			TCON_DEBUG("LineOD RGB bin format is Seperate\n");
			...
			case LINE_OD_TABLE_UNIT_SIZE_8:
				while (index < u16TableSize) {
					...
					pu16GLut[index] = pu8TconTab[index + u16TargetIndex + LINE_OD_TABLE_SIZE_19X19];
					pu16BLut[index] = pu8TconTab[index + u16TargetIndex + (2 * LINE_OD_TABLE_SIZE_19X19)];
					index++;
					udelay(1);
				}
				u16TargetIndex += 3 * LINE_OD_TABLE_SIZE_19X19;//bin fil;e shift 361 unit
				break;
