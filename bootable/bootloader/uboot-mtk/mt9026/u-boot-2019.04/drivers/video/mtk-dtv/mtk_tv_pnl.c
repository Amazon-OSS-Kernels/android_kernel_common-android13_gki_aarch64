...
static int _parse_dts_info(struct udevice *dev)
{
    ...
    if (!ofnode_valid(info_node)) {
		...
		/* LVDS setting */
		ofnode_read_u32(info_node, PANEL_SWAP_LVDS_POL, &tmp);
		priv->lvds_info.panel_swap_lvds_pol = (!((bool)tmp));
		ofnode_read_u32(info_node, PANEL_SWAP_LVDS_CH, &tmp);
		priv->lvds_info.panel_swap_lvds_ch = (bool)tmp;
		ofnode_read_u32(info_node, PANEL_PDP_10BIT, &tmp);
		priv->lvds_info.panel_pdp_10bit = (bool)tmp;
		ofnode_read_u32(info_node, PANEL_LVDS_TI_MODE, &tmp);
		priv->lvds_info.panel_lvds_ti_mode = (bool)tmp;
		ofnode_read_u32(info_node, PANEL_TI_BIT_MODE, &tmp);
		priv->lvds_info.ti_bit_mode = tmp;
		ofnode_read_u32(info_node, VCC_TO_CUSTIC_DLY, &tmp);
		priv->tcon_info.vcc_to_custic_delay = tmp;
		UBOOT_TRACE("\n priv->tcon_info.vcc_to_custic_delay=%d\n ", priv->tcon_info.vcc_to_custic_delay);
		UBOOT_TRACE("\n priv->lvds_info.panel_swap_lvds_pol=%d\n ", priv->lvds_info.panel_swap_lvds_pol);
		UBOOT_TRACE("\n priv->lvds_info.panel_swap_lvds_ch=%d\n ", priv->lvds_info.panel_swap_lvds_ch);
		UBOOT_TRACE("\n priv->lvds_info.panel_pdp_10bit=%d\n ", priv->lvds_info.panel_pdp_10bit);
		UBOOT_TRACE("\n priv->lvds_info.panel_lvds_ti_mode=%d\n ", priv->lvds_info.panel_lvds_ti_mode);
		UBOOT_TRACE("\n priv->lvds_info.ti_bit_mode=%d\n ", priv->lvds_info.ti_bit_mode);
    }
    ...
    if(priv->tcon_info.bUsingTCON)
    {
		...
		tcon_vrr_od_node = dev_read_subnode(dev, PNL_TCON_VRR_OD_INFO_NODE);
		if (!ofnode_valid(tcon_vrr_od_node)) {
			UBOOT_ERROR("Read subnode (%s) FAIL\n", PNL_TCON_VRR_OD_INFO_NODE);
		} else {
			ofnode_read_u32(tcon_vrr_od_node, VRR_OD_EN_TAG, &tmp);
			priv->tcon_info.bvrr_od_en = (bool)tmp;
		}
	...
	if (priv->typ_htt != 0 && priv->typ_vtt != 0 ) {
		...
		if (IS_OUT_HD(priv->de_width, priv->de_height)) {
			if (IS_VFREQ_60HZ_GROUP(vfreq)) {
...
static int mtk_panel_probe(struct udevice *dev)
{
	...
	if (priv->pnl_lib_version == BOOT_PNL_VERSION0600)
	{
		UBOOT_DEBUG("[PNL]BOOT_PNL_VERSION0600\n");
...
#endif
		UBOOT_DEBUG("[PNL]BOOT_PNL_VERSION OLD\n");
