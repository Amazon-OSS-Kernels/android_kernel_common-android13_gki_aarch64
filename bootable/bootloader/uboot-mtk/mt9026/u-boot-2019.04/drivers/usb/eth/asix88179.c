...
#define ASIX_USB_BULK_RECV_TIMEOUT 500
...
/* Monitor the Link Status - this polling keeps the PHY domain active and
 * prevents some versions of the dongle from powering-off after a period
 * of inactivity. The Linux kernel driver achieves this by using the
 * INTERRUPT endpoint IRQs. This interrupt handling is not implemented in
 * U-BOOT.
 */
static void asix_check_link(struct ueth_data *ueth)
{
	u16 status;

	asix_read_cmd(ueth, AX_ACCESS_PHY, AX88179_PHY_ID, MII_BMSR, 2, &status);
	if (!(status & BMSR_LSTATUS)) {
		printf("%s: Link status is: %d\n", __func__, !!(status & BMSR_LSTATUS));
	}
}
...
static int ax88179_eth_probe(struct udevice *dev)
{
	...
	if (asix_basic_reset(&priv->ueth, priv))
		debug("%s: Failed initial reset\n", __func__);
