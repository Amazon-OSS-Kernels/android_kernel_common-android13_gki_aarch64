...
static int mtu3_gadget_dequeue(struct usb_ep *ep, struct usb_request *req)
{
	...
	dev_dbg(mep->mtu->dev, "%s : req=%p\n", __func__, req);

	spin_lock_irqsave(&mep->mtu->lock, flags);
	...
	if (r != mreq) {
		dev_dbg(mep->mtu->dev, "req=%p not queued to %s\n", req, ep->name);
		ret = -EINVAL;
		goto done;
	}
	...
	spin_unlock_irqrestore(&mep->mtu->lock, flags);
...
static int mtu3_gadget_ep_set_halt(struct usb_ep *ep, int value)
{
	...
	dev_dbg(mep->mtu->dev, "%s : %s...", __func__, ep->name);
	...
	if (value) {
		 ...
		 */
		if (mreq) {
			dev_dbg(mep->mtu->dev, "req in progress, cannot halt %s\n",
				ep->name);
			ret = -EAGAIN;
			goto done;
		}
		       ...
		       u32 epnum, u32 is_in)
{
	...
	ret = sprintf(mep->name, "ep%d%s", epnum,
		!epnum ? "" : (is_in ? "in" : "out"));
	if (ret <0) return;
