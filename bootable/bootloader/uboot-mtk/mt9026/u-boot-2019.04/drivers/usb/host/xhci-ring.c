							...
							u32 ep_state)
{
	...
	case EP_STATE_HALTED:
		...
		puts("WARN halted endpoint, return with error\n");
		return -EINVAL;
#else
...
union xhci_trb *xhci_wait_for_event(struct xhci_ctrl *ctrl, trb_type expected)
{
	...
	do {
		...
		/* TODO: remove this once enumeration has been reworked */
			 ...
			 */
			index = le32_to_cpu(event->generic.field[0]) >> 24;
			if (!index)
			    continue;

			reg = xhci_readl((&hcor->portregs[index - 1].or_portsc));

			if (!(reg & PORT_CONNECT)) {
			    printf("XHCI event TRB, Port status change...(%08x %08x %08x %08x)\n",
			...
			int length, void *buffer)
{
	...
	union xhci_trb *event;
	struct xhci_generic_trb *last_transfer_trb_addr;
...
again:
	...
	if ((uintptr_t)(le64_to_cpu(event->trans_event.buffer))
		...
		available_length -=
			(int)EVENT_TRB_LEN(le32_to_cpu(event->trans_event.transfer_len));
		printf("Short packet, actual data %d.\n", available_length);
		xhci_acknowledge_event(ctrl);
		goto again;
	}
