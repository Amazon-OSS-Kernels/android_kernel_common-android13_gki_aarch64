...
static int xhci_reset_endpoint(struct usb_device *udev, unsigned int ep_index)
{
	...
	xhci_queue_command(ctrl, NULL, udev->slot_id, 0,
			   TRB_RESET_EP);
	event = xhci_wait_for_event(ctrl, TRB_COMPLETION);
	WARN_ON(TRB_TO_SLOT_ID(le32_to_cpu(event->event_cmd.flags))
		!= udev->slot_id);
...
static int xhci_set_configuration(struct usb_device *udev)
{
	...
	for (cur_ep = 0; cur_ep < num_of_ep; cur_ep++) {
		...
		if (usb_endpoint_xfer_int(endpt_desc)) {
			/* [MTK] patched for interrupt pipe! */
			ep_ctx[ep_index]->reserved[0] |= 1;
		}
	}
				    ...
				    struct devrequest *setup, int root_portnr)
{
	...
	if (setup->requesttype == USB_RECIP_ENDPOINT &&
		...
		if (!xhci_reset_endpoint(udev, ep_index))
			puts("Failed to reset halted xHCI endpoint\n");
	}
	return ret;
#else
