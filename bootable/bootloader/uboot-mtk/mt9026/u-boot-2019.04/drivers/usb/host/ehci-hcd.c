...
#ifdef CONFIG_USB_EHCI_MSTAR
		td->qt_buffer[idx] = VA2PA(td->qt_buffer[idx]);
#endif
...
#ifdef CONFIG_USB_EHCI_MSTAR
	ctrl->qh_list.qh_link = VA2PA(ctrl->qh_list.qh_link);
#endif
...
#ifdef CONFIG_USB_EHCI_MSTAR
	ehci_writel(&ctrl->hcor->or_asynclistaddr, VA2PA(virt_to_phys(&ctrl->qh_list)));
#else
...
#if !defined(CONFIG_USB_EHCI_FARADAY) && !defined(CONFIG_USB_EHCI_MSTAR)
...
static int ehci_common_init(struct ehci_ctrl *ctrl, uint tweaks)
{
...
#ifdef CONFIG_USB_EHCI_MSTAR
	qh_list->qh_link = VA2PA(qh_list->qh_link);
#endif
	...
	/* Set async. queue head pointer. */
#ifdef CONFIG_USB_EHCI_MSTAR
	ehci_writel(&ctrl->hcor->or_asynclistaddr, VA2PA(virt_to_phys(qh_list)));
#else
