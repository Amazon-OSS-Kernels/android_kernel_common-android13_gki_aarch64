...
#define MAX_STRING_MANUFACTURER	32
static char product[MAX_STRING_PRODUCT];
static char g_dnl_serial[MAX_STRING_SERIAL];
static char manufacturer[MAX_STRING_MANUFACTURER];
...
int GetEnvProduct(void)
{
	char *sptr = NULL;
	int  slen, result;

	memset(product, 0, MAX_STRING_PRODUCT);
	sptr = env_get("android_product");
	if (sptr) {
		slen = strlen(sptr);
		if (slen > (MAX_STRING_PRODUCT - 1))
			slen = MAX_STRING_PRODUCT - 1;
		memcpy(product, sptr, slen);
	} else {
		result = snprintf(product, MAX_STRING_PRODUCT - 1, "%s", "USB download gadget");
		if (result < 0)
			printf("snprintf error\n");
	}
	printf("EvnProduct:%s\n", product);

	sptr = env_get("android_manufacturer");
	if (sptr) {
		slen = strlen(sptr);
		if (slen > (MAX_STRING_MANUFACTURER - 1))
			slen = MAX_STRING_MANUFACTURER - 1;
		memcpy(manufacturer, sptr, slen);
	} else {
		result = snprintf(manufacturer, MAX_STRING_MANUFACTURER - 1, "%s", CONFIG_USB_GADGET_MANUFACTURER);
		if (result < 0)
			printf("snprintf error\n");
	}
	printf("EvnManufacturer:%s\n", manufacturer);

	sptr = env_get("serialno");
	if (sptr) {
		slen = strlen(sptr);
		if (slen > MAX_STRING_SERIAL - 1)
			slen = MAX_STRING_SERIAL - 1;
		memcpy(g_dnl_serial, sptr, slen);
	}
