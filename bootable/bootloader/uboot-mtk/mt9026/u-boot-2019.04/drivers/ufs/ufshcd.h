...
struct ufs_hba_variant_ops {
	...
	#ifdef __UBOOT__
	void (*rpmb_hmac)(struct ufs_hba *hba, unsigned char *key,
			unsigned char *buff, int len, unsigned char *output);
	#endif
};
...
#define	UFSHCD_CAP_CRYPTO  (1 << 6)
	...
	u8 bNumberLU;
	u16 wManufacturerID;
	u16 wManufactureDate;
	u8 iManufacturerName;
	u8 iProductName;
	u8 iSerialNumber;
	u8 iProductRevisionLevel;
	u8* sManufacturerName;
	u8* sProductName;
	u8* sSerialNumber;
	u8* sProductRevisionLevel;
	#endif
};
...
#endif
#ifdef __UBOOT__
unsigned long ufshcd_wait_for_completion_timeout(struct ufs_hba *hba, struct completion *x,
							unsigned long timeout_us);
unsigned long ufshcd_wait_event_timeout(struct ufs_hba *hba, int slot,
							unsigned long timeout_us);
