...
/*
 * MediaTek clock driver for MT5896 SoC
 *
...
#include <asm/io.h>
#include <dt-bindings/clock/mt5896-clk.h>

#include "clk-mtk.h"

#define MT5896_CLKSQ_STB_CON0		0x20
#define MT5896_PLL_ISO_CON0		0x2c
#define MT5896_PLL_FMAX			(2500UL * MHZ)
#define MT5896_CON0_RST_BAR		BIT(24)
...
#define PLL(_id, _reg, _pwr_reg, _en_mask, _flags, _pcwbits, _pd_reg,	\
		...
		.pwr_reg = _pwr_reg,					\
		.en_mask = _en_mask,					\
		.rst_bar_mask = MT5896_CON0_RST_BAR,			\
		.fmax = MT5896_PLL_FMAX,				\
		.flags = _flags,					\
		.pcwbits = _pcwbits,					\
		.pd_reg = _pd_reg,					\
		.pd_shift = _pd_shift,					\
...
static const struct mtk_clk_tree mt5896_clk_tree = {
	.xtal_rate = 40 * MHZ,
	.xtal2_rate = 20 * MHZ,
...
static int mt5896_mcucfg_probe(struct udevice *dev)
{
	void __iomem *base;
...
static int mt5896_apmixedsys_probe(struct udevice *dev)
{
	struct mtk_clk_priv *priv = dev_get_priv(dev);
	int ret;

	ret = mtk_common_clk_init(dev, &mt5896_clk_tree);
	if (ret)
		return ret;

	/* reduce clock square disable time */
	writel(0x501, priv->base + MT5896_CLKSQ_STB_CON0);
	/* extend pwr/iso control timing to 1us */
	writel(0x80008, priv->base + MT5896_PLL_ISO_CON0);

	return 0;
}

static int mt5896_topckgen_probe(struct udevice *dev)
{
	return mtk_common_clk_init(dev, &mt5896_clk_tree);
}

static int mt5896_infracfg_probe(struct udevice *dev)
{
	return mtk_common_clk_gate_init(dev, &mt5896_clk_tree, infra_cgs);
}

static int mt5896_pericfg_probe(struct udevice *dev)
{
	return mtk_common_clk_gate_init(dev, &mt5896_clk_tree, peri_cgs);
}

static int mt5896_ethsys_probe(struct udevice *dev)
{
	return mtk_common_clk_gate_init(dev, &mt5896_clk_tree, eth_cgs);
}

static int mt5896_ethsys_bind(struct udevice *dev)
{
	int ret = 0;
...
static int mt5896_sgmiisys_probe(struct udevice *dev)
{
	return mtk_common_clk_gate_init(dev, &mt5896_clk_tree, sgmii_cgs);
}

static const struct udevice_id mt5896_apmixed_compat[] = {
	{ .compatible = "mediatek,mt5896-apmixedsys" },
	{ }
};

static const struct udevice_id mt5896_topckgen_compat[] = {
	{ .compatible = "mediatek,mt5896-topckgen" },
	{ }
};

static const struct udevice_id mt5896_infracfg_compat[] = {
	{ .compatible = "mediatek,mt5896-infracfg", },
	{ }
};

static const struct udevice_id mt5896_pericfg_compat[] = {
	{ .compatible = "mediatek,mt5896-pericfg", },
	{ }
};

static const struct udevice_id mt5896_ethsys_compat[] = {
	{ .compatible = "mediatek,mt5896-ethsys", },
	{ }
};

static const struct udevice_id mt5896_sgmiisys_compat[] = {
	{ .compatible = "mediatek,mt5896-sgmiisys", },
	{ }
};

static const struct udevice_id mt5896_mcucfg_compat[] = {
	{ .compatible = "mediatek,mt5896-mcucfg" },
	{ }
};
...
U_BOOT_DRIVER(mtk_mcucfg) = {
	.name = "mt5896-mcucfg",
	.id = UCLASS_SYSCON,
	.of_match = mt5896_mcucfg_compat,
	.probe = mt5896_mcucfg_probe,
	.flags = DM_FLAG_PRE_RELOC,
};

U_BOOT_DRIVER(mtk_clk_apmixedsys) = {
	.name = "mt5896-clock-apmixedsys",
	.id = UCLASS_CLK,
	.of_match = mt5896_apmixed_compat,
	.probe = mt5896_apmixedsys_probe,
...
U_BOOT_DRIVER(mtk_clk_topckgen) = {
	.name = "mt5896-clock-topckgen",
	.id = UCLASS_CLK,
	.of_match = mt5896_topckgen_compat,
	.probe = mt5896_topckgen_probe,
	.priv_auto_alloc_size = sizeof(struct mtk_clk_priv),
...
U_BOOT_DRIVER(mtk_clk_infracfg) = {
	.name = "mt5896-clock-infracfg",
	.id = UCLASS_CLK,
	.of_match = mt5896_infracfg_compat,
	.probe = mt5896_infracfg_probe,
...
U_BOOT_DRIVER(mtk_clk_pericfg) = {
	.name = "mt5896-clock-pericfg",
	.id = UCLASS_CLK,
	.of_match = mt5896_pericfg_compat,
	.probe = mt5896_pericfg_probe,
	.priv_auto_alloc_size = sizeof(struct mtk_cg_priv),
	.ops = &mtk_clk_gate_ops,
	.flags = DM_FLAG_PRE_RELOC,
};
...
U_BOOT_DRIVER(mtk_clk_ethsys) = {
	.name = "mt5896-clock-ethsys",
	.id = UCLASS_CLK,
	.of_match = mt5896_ethsys_compat,
	.probe = mt5896_ethsys_probe,
	.bind = mt5896_ethsys_bind,
	.priv_auto_alloc_size = sizeof(struct mtk_cg_priv),
	.ops = &mtk_clk_gate_ops,
};

U_BOOT_DRIVER(mtk_clk_sgmiisys) = {
	.name = "mt5896-clock-sgmiisys",
	.id = UCLASS_CLK,
	.of_match = mt5896_sgmiisys_compat,
	.probe = mt5896_sgmiisys_probe,
	.priv_auto_alloc_size = sizeof(struct mtk_cg_priv),
	.ops = &mtk_clk_gate_ops,
};
