...
#define GMAC_DEV_NAME		"ethernet@1c700000"
...
struct mtk_dtv_gmac_reg_ops {
	...
	void (*phy_power_down)(struct mtk_dtv_gmac_priv *priv);
};
...
/* ext APIs */
int mtk_dtv_gmac_hw_power_down_clk(void)
{
	struct udevice *dev = NULL;
	struct mtk_dtv_gmac_priv *priv = NULL;
	struct mtk_dtv_gmac_reg_ops *reg_ops = NULL;
	int ret = 0;

	ret = uclass_get_device_by_name(UCLASS_ETH, GMAC_DEV_NAME, &dev);
	...
	reg_ops = priv->reg_ops;

	reg_ops->phy_power_down(priv);
...
static void mtk_dtv_gmac_phy_hw_init_22(struct mtk_dtv_gmac_priv *priv)
{
	...
	if (priv->phy_comp == PHY_COMP_U) {
		//wriu    0x0422ee              0x66
		reg_read = 0x66;
		MTK_W8(REG_OFFSET_77_L(BANK_BASE_ALBANY_2), reg_read);
		//wriu    0x0422ef              0x66
		reg_read = 0x66;
		MTK_W8(REG_OFFSET_77_H(BANK_BASE_ALBANY_2), reg_read);
	}
}
...
static void mtk_dtv_gmac_hw_power_down_clk_7_12(struct mtk_dtv_gmac_priv *priv)
{
	//wriu     0x042104             0x31
	MTK_W8(REG_OFFSET_02_L(BANK_BASE_ALBANY_1), 0x31);
	//wriu     0x0421fc             0x02
	MTK_W8(REG_OFFSET_7E_L(BANK_BASE_ALBANY_1), 0x02);
	//wriu     0x0421fd             0x01
	MTK_W8(REG_OFFSET_7E_H(BANK_BASE_ALBANY_1), 0x01);
	//wriu     0x0422a1             0x30
	MTK_W8(REG_OFFSET_50_H(BANK_BASE_ALBANY_2), 0x30);
	//wriu     0x0421cc             0x50
	MTK_W8(REG_OFFSET_66_L(BANK_BASE_ALBANY_1), 0x50);
	//wriu     0x0421bb             0xc4
	MTK_W8(REG_OFFSET_5D_H(BANK_BASE_ALBANY_1), 0xc4);
	//wriu     0x04223a             0xf3
	MTK_W8(REG_OFFSET_1D_L(BANK_BASE_ALBANY_2), 0xf3);
	//wriu     0x0422f1             0x3c
	MTK_W8(REG_OFFSET_78_H(BANK_BASE_ALBANY_2), 0x3c);
	//wriu     0x0422f3             0x0f
	MTK_W8(REG_OFFSET_79_H(BANK_BASE_ALBANY_2), 0x0f);
	//wriu     0x0422c5             0x40
	MTK_W8(REG_OFFSET_62_H(BANK_BASE_ALBANY_2), 0x40);
	//wriu     0x042230             0x53
	MTK_W8(REG_OFFSET_18_L(BANK_BASE_ALBANY_2), 0x53);
	//wriu     0x0422f7             0x08
	MTK_W8(REG_OFFSET_7B_H(BANK_BASE_ALBANY_2), 0x08);
	//wriu     0x0422aa             0x03
	MTK_W8(REG_OFFSET_55_L(BANK_BASE_ALBANY_2), 0x03);
	//wriu     0x0422f4             0x1a
	MTK_W8(REG_OFFSET_7A_L(BANK_BASE_ALBANY_2), 0x1a);
	//wriu     0x042104             0x11
	MTK_W8(REG_OFFSET_02_L(BANK_BASE_ALBANY_1), 0x11);

	UBOOT_INFO("%s:%d\n", __func__, __LINE__);
}

static void mtk_dtv_gmac_hw_power_down_clk_22(struct mtk_dtv_gmac_priv *priv)
{
	//wriu     0x042104             0x31
	MTK_W8(REG_OFFSET_02_L(BANK_BASE_ALBANY_1), 0x31);
	//wriu     0x0421fc             0x02
	...
	MTK_W8(REG_OFFSET_7E_H(BANK_BASE_ALBANY_1), 0x01);
	//wriu     0x0422a1             0xb0
	MTK_W8(REG_OFFSET_50_H(BANK_BASE_ALBANY_2), 0xb0);
	//wriu     0x0421cc             0x50
	MTK_W8(REG_OFFSET_66_L(BANK_BASE_ALBANY_1), 0x50);
	//wriu     0x0421bb             0xc4
	MTK_W8(REG_OFFSET_5D_H(BANK_BASE_ALBANY_1), 0xc4);
...
static int mtk_dtv_gmac_ofdata_to_platdata(struct udevice *dev)
{
	...
	get_val = 0;
	if (dev_read_u32(dev, "phy-comp", &get_val)) {
		priv->phy_comp = PHY_COMP_T;
		UBOOT_DEBUG("gmac set phy_comp to default value (%d)\n",
			    priv->phy_comp);
	} else {
		priv->phy_comp = get_val;
		UBOOT_DEBUG("gmac phy_comp (%d)\n",
			    priv->phy_comp);
	}
