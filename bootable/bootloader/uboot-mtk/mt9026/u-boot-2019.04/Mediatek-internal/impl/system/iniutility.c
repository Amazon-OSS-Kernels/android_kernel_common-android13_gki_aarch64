...
EXIT:
    ...
    if (ret < 0)
    {
        add_bootargs(key_mode, (char *)dlg_boot0, 0);
        add_bootargs(key_mode_gop, (char *)dlg_boot_gop0, 0);
    ...
    else
    {
        if(info->panel_dlg)
        {
            add_bootargs(key_mode, (char *)dlg_boot1, 0);
            add_bootargs(key_mode_gop, (char *)dlg_boot_gop1, 0);
            UBOOT_INFO("add_bootargs %s\n", dlg_boot1);
        }
        else
        {
