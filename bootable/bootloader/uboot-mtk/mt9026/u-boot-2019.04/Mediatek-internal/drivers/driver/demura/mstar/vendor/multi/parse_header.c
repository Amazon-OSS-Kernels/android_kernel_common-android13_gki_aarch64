...
// { SIO
MS_BOOL load_vendor_header_sio(SIO_DEMURA_BIN_HEADER *phdr)
{
    MS_U8 *buffer = (MS_U8 *)dmalloc(SIO_HEADER_LEN);

    if (buffer == NULL)
    {
        UBOOT_ERROR("In %s, malloc 0x%x byte failed!\n", __FUNCTION__, (unsigned int)SIO_HEADER_LEN);
        return FALSE;
    }

    MS_BOOL bRet = read_spi_flash(buffer, SIO_HEADER_START, SIO_HEADER_LEN);
    if (bRet == FALSE)
    {
        UBOOT_ERROR("Read CSOT_HIMAX Header from spi flash failed !\n");
        dfree(buffer);
...
MS_BOOL parse_vendor_header_sio(SIO_DEMURA_BIN_HEADER *phdr)
{
    MS_U8  *pdata = phdr->hdr_buffer;
    MS_U32 *pHeader = (MS_U32 *)phdr->hdr_buffer;
    MS_U8 bBigEndian = SIO_DEMURA_IS_BIG_ENDIAN(pdata);
    MS_U16 tmp;
    MS_U32 u32RegChkSum = 0;
    MS_U8 *pu8Reg = (MS_U8 *) (&pHeader[SIO_VD_CTRL_REGISTER]);
    phdr->lut_buffer = NULL;
    phdr->lut_offset = SIO_HEADER_LEN;
    phdr->lut_length = 0;
    phdr->HEADER_REG_SIZE = SIO_DEMURA_U32_TO_U32(bBigEndian, pHeader[SIO_HEADER_REG_SIZE]);
    phdr->HEADER_REG_CHECKSUM = SIO_DEMURA_U32_TO_U32(bBigEndian, pHeader[SIO_HEADER_REG_CHKSUM]);
    phdr->HEADER_LUT_SIZE = SIO_DEMURA_U32_TO_U32(bBigEndian, pHeader[SIO_HEADER_LUT_SIZE]);
    phdr->HEADER_LUT_CHECKSUM = SIO_DEMURA_U32_TO_U32(bBigEndian, pHeader[SIO_HEADER_LUT_CHKSUM]);
    phdr->HEADER_VD_CTRL = SIO_DEMURA_U32_TO_U32(bBigEndian, pHeader[SIO_VD_CTRL_REGISTER]);
    SIO_DEMURA_U32_TO_U16(bBigEndian, pHeader[SIO_REG_BLOCK_SIZE], tmp, phdr->DEMURA_CTRL_REG_BLOCK_SIZE);
    SIO_DEMURA_U32_TO_U8_AND_U16(bBigEndian, pHeader[SIO_REG_PLANE_NUM],
        tmp, tmp, phdr->DEMURA_CTRL_REG_PLANE_NUM);
    SIO_DEMURA_U32_TO_U16(bBigEndian, pHeader[SIO_REG_LOW_R_LIMIT],
        phdr->DEMURA_CTRL_REG_LOW_R_LIMIT, tmp);
    SIO_DEMURA_U32_TO_U16(bBigEndian, pHeader[SIO_REG_PLANE_R0_LIMIT],
        tmp, phdr->DEMURA_CTRL_REG_PLANE_LEVEL[0]);
    SIO_DEMURA_U32_TO_U16(bBigEndian, pHeader[SIO_REG_PLANE_R1_LIMIT],
        phdr->DEMURA_CTRL_REG_PLANE_LEVEL[1],
        phdr->DEMURA_CTRL_REG_PLANE_LEVEL[2]);
    SIO_DEMURA_U32_TO_U16(bBigEndian, pHeader[SIO_REG_PLANE_R1_LIMIT + 1],
        phdr->DEMURA_CTRL_REG_PLANE_LEVEL[3],
        phdr->DEMURA_CTRL_REG_PLANE_LEVEL[4]);
    SIO_DEMURA_U32_TO_U16(bBigEndian, pHeader[SIO_REG_PLANE_R1_LIMIT + 2],
        phdr->DEMURA_CTRL_REG_PLANE_LEVEL[5],
        phdr->DEMURA_CTRL_REG_PLANE_LEVEL[6]);
    SIO_DEMURA_U32_TO_U16(bBigEndian, pHeader[SIO_REG_PLANE_R7_LIMIT],
        phdr->DEMURA_CTRL_REG_PLANE_LEVEL[7], tmp);
    SIO_DEMURA_U32_TO_U16(bBigEndian, pHeader[SIO_REG_HIGH_R_LIMIT],
        tmp, phdr->DEMURA_CTRL_REG_HIGH_R_LIMIT);

    if (phdr->HEADER_REG_SIZE != 0x200)
    {
        return FALSE;
    }

    //calcultion bin header checksum
    u32RegChkSum = 0;
    for (tmp = 0; tmp < phdr->HEADER_REG_SIZE; tmp += 2)
    {
        u32RegChkSum += (((MS_U16)pu8Reg[tmp]) << 8) | pu8Reg[tmp + 1];
    }
    u32RegChkSum = 0xFFFF & u32RegChkSum;
    UBOOT_TRACE("u32RegChkSum=0x%04x, HEADER_REG_CHECKSUM=0x%04x\n", u32RegChkSum, phdr->HEADER_REG_CHECKSUM);

    if (phdr->HEADER_REG_CHECKSUM != u32RegChkSum) // chk twice due to no answer from cus
    {
        UBOOT_ERROR("bin header checksum not match u32RegChkSum=0x%04x, HEADER_REG_CHECKSUM=0x%04x\n", u32RegChkSum, phdr->HEADER_REG_CHECKSUM);
    }

     // size check
    switch (phdr->DEMURA_CTRL_REG_BLOCK_SIZE)
    {
        case 0:
            phdr->HBlock     = 2;
            phdr->VBlock     = 32;
        ...
        case 7:
            ...
            break;
        default:
            UBOOT_ERROR("Error block size\n");
            phdr->HBlock     = 1;
            phdr->VBlock     = 1;
            break;
    }
    phdr->HLutNum = (3840 / phdr->HBlock) + 1;
    phdr->VLutNum = (2160 / phdr->VBlock) + 1;
#if DGB_DEMURA_VENDOR_LOG
    {
        int i = 0;
        UBOOT_TRACE("BLOCK:%u x %u\n", phdr->HBlock, phdr->VBlock);
        UBOOT_TRACE("LutNum:%u x %u\n", phdr->HLutNum, phdr->VLutNum);
        UBOOT_TRACE("HEADER_REG_SIZE:0x%x\n", phdr->HEADER_REG_SIZE);
        UBOOT_TRACE("HEADER_REG_CHECKSUM:0x%x\n", phdr->HEADER_REG_CHECKSUM);
        UBOOT_TRACE("HEADER_LUT_SIZE:0x%x\n", phdr->HEADER_LUT_SIZE);
        UBOOT_TRACE("HEADER_LUT_CHECKSUM:0x%x\n", phdr->HEADER_LUT_CHECKSUM);
        UBOOT_TRACE("HEADER_VD_CTRL:0x%x\n", phdr->HEADER_VD_CTRL);
        UBOOT_TRACE("DEMURA_CTRL_REG_BLOCK_SIZE:0x%x\n", phdr->DEMURA_CTRL_REG_BLOCK_SIZE);
        UBOOT_TRACE("DEMURA_CTRL_REG_PLANE_NUM:0x%x\n", phdr->DEMURA_CTRL_REG_PLANE_NUM);
        UBOOT_TRACE("DEMURA_CTRL_REG_LOW_R_LIMIT:0x%x\n", phdr->DEMURA_CTRL_REG_LOW_R_LIMIT);
        UBOOT_TRACE("DEMURA_CTRL_REG_HIGH_R_LIMIT:0x%x\n", phdr->DEMURA_CTRL_REG_HIGH_R_LIMIT);
        for (i = 0; i < SIO_DEMURA_MAX_PLANE; i++)
        {
            UBOOT_TRACE("plane[%2u]:0x%x\n", i, phdr->DEMURA_CTRL_REG_PLANE_LEVEL[i]);
        }
    }
#endif
...
MS_BOOL load_vendor_lut_sio(SIO_DEMURA_BIN_HEADER *phdr)
{
    MS_U8  *lut_buf = NULL;
    MS_U32 tmp;
    MS_U32 u32lutChkSum = 0;

    UBOOT_TRACE("load_vendor_lut_sio %x,%X\n", phdr->lut_length, phdr->HEADER_LUT_SIZE);

    lut_buf = dmalloc(phdr->HEADER_LUT_SIZE);
    CHECK_DMALLOC_SPACE(lut_buf, (unsigned int)phdr->HEADER_LUT_SIZE);
    if (read_spi_flash(lut_buf, SIO_LUT_START, phdr->HEADER_LUT_SIZE) == FALSE)
    {
        UBOOT_ERROR("Read ROKU Lut from spi flash failed !\n");
        dfree(lut_buf);
        return FALSE;
    }

    UBOOT_TRACE("=====SIO LUT CRC IN=====\n");

    //calcultion bin header checksum
    u32lutChkSum = 0;
    for (tmp = 0; tmp < (phdr->HEADER_LUT_SIZE); tmp += 2)
    {
        u32lutChkSum += (((MS_U16)lut_buf[tmp]) << 8) | lut_buf[tmp + 1];
    }
    u32lutChkSum = 0xFFFF & u32lutChkSum;

    UBOOT_TRACE("u32lutChkSum=0x%04x, LUT_CHECKSUM=0x%04x\n", u32lutChkSum, phdr->HEADER_LUT_CHECKSUM);

    if (u32lutChkSum != phdr->HEADER_LUT_CHECKSUM)
    {
        UBOOT_ERROR("BOE ESWIN LUT CRC FAIL %x %x\n", u32lutChkSum, phdr->HEADER_LUT_CHECKSUM);
        dfree(lut_buf);
        return FALSE;
    }
    UBOOT_TRACE("=====SIO LUT CRC OK=====\n");
    phdr->lut_buffer = lut_buf;
