...
// { Start SIO
#define SIO_DEMURA_MAX_PLANE     (8)
#define SIO_HEADER_START      (0x0063000)
#define SIO_HEADER_LEN        (0x220)
#define SIO_LUT_START         (SIO_HEADER_START + SIO_HEADER_LEN)
#define SIO_HEADER_REG_SIZE       (0)
#define SIO_HEADER_REG_CHKSUM     (1)
#define SIO_HEADER_LUT_SIZE       (2)
#define SIO_HEADER_LUT_CHKSUM     (3)
#define SIO_VD_CTRL_REGISTER      (8)
#define SIO_DEMURA_REGISTER_START (0x10)
#define SIO_VD_CTRL_REGISTER_SCL  (SIO_VD_CTRL_REGISTER + 1)
#define SIO_DEMURA_CTRL_START     (SIO_VD_CTRL_REGISTER + SIO_DEMURA_REGISTER_START)
#define SIO_REG_BLOCK_SIZE        (SIO_DEMURA_CTRL_START + 0)
#define SIO_REG_PLANE_NUM         (SIO_DEMURA_CTRL_START + 1)
#define SIO_REG_LOW_R_LIMIT       (SIO_DEMURA_CTRL_START + 2)
#define SIO_REG_PLANE_R0_LIMIT    (SIO_DEMURA_CTRL_START + 3)
#define SIO_REG_PLANE_R1_LIMIT    (SIO_DEMURA_CTRL_START + 4)
#define SIO_REG_PLANE_R3_LIMIT    (SIO_DEMURA_CTRL_START + 5)
#define SIO_REG_PLANE_R5_LIMIT    (SIO_DEMURA_CTRL_START + 6)
#define SIO_REG_PLANE_R7_LIMIT    (SIO_DEMURA_CTRL_START + 7)
#define SIO_REG_HIGH_R_LIMIT      (SIO_DEMURA_CTRL_START + 16)

#define SIO_DEMURA_IS_BIG_ENDIAN(phdr) \
    (((phdr[0] == 0x00) && (phdr[1] == 0x00) && (phdr[2] == 0x02) && (phdr[3] == 0x00))?1:0)

#define SIO_DEMURA_U8ARRAY_TO_U32(hh, hl, lh, ll) \
    (((MS_U32) hh << 24) | ((MS_U32) hl << 16) | ((MS_U32) lh << 8) | (MS_U32) ll)

#define SIO_DEMURA_U8ARRAY_TO_U16(h, l) (((u16) h << 8) | l)
#define SIO_DEMURA_U32_TO_U32(bBigEndian, u32val) \
    ({ \
        MS_U8 *data = (MS_U8 *)&(u32val); \
        MS_U32 ret; \
        if (bBigEndian) {ret = SIO_DEMURA_U8ARRAY_TO_U32(data[0], data[1], data[2], data[3]);} \
        else {ret = SIO_DEMURA_U8ARRAY_TO_U32(data[3], data[2], data[1], data[0]);}\
        /*UBOOT_TRACE("%s@%d %X{%02X %02X %02X %02X}\n",__FUNCTION__,__LINE__, ret, data[3], data[2],      data[1], data[0]); */\
        ret; \
    })

#define SIO_DEMURA_U32_TO_U16(bBigEndian, u32val, u16val_l, u16val_h) \
    ({ \
        MS_U8 *data = (MS_U8 *)&(u32val); \
        if (bBigEndian) {\
            u16val_h = (((MS_U16)data[0]) << 8) | data[1]; \
            u16val_l = (((MS_U16)data[2]) << 8) | data[3]; \
        }\
        else {\
            u16val_h = (((MS_U16)data[3]) << 8) | data[2]; \
            u16val_l = (((MS_U16)data[1]) << 8) | data[0]; \
        }\
        /*UBOOT_TRACE("%s@%d {%02X} {%02X} %02X %02X %02X %02X\n",__FUNCTION__,__LINE__, u16val_h, u16val_l, data[0], data[1], data[2],data[3]);*/ \
    })

#define SIO_DEMURA_U32_TO_U8_AND_U16(bBigEndian, u32val, u8ret0, u8ret1, u16ret) \
    ({ \
        MS_U8 *data = (MS_U8 *)&(u32val); \
        if (bBigEndian) {\
            u16ret = (((MS_U16) data[0]) << 8) | (MS_U16)data[1]; \
            u8ret1 = data[2]; \
            u8ret0 = data[3]; \
        }\
        else {\
            u16ret = (((MS_U16) data[3]) << 8) | (MS_U16)data[2]; \
            u8ret1 = data[1]; \
            u8ret0 = data[0]; \
        }\
        /* UBOOT_TRACE("%s@%d {%02X}{%02X}{%02X}%02X %02X %02X %02X\n",__FUNCTION__,__LINE__, u16ret, u8ret1, u8ret0, data[0], data[1], data[2], data[3]);*/ \
    })

#define SIO_DEMURA_U32_SWAP_OFFSET(n) (3 - 2 * (n % 4)) //((3 - (n % 4)) - (n % 4))
#define SIO_DEMURA_U32_ALIGN_4BYTES(n) ((n+0x03) & 0xfffffffc)

typedef struct
{
    MS_U32 HEADER_REG_SIZE;
    MS_U32 HEADER_REG_CHECKSUM;
    MS_U32 HEADER_LUT_SIZE;
    MS_U32 HEADER_LUT_CHECKSUM;
    MS_U32 HEADER_VD_CTRL;
    MS_U16 DEMURA_CTRL_REG_BLOCK_SIZE;
    MS_U16 DEMURA_CTRL_REG_PLANE_NUM;
    MS_U16 DEMURA_CTRL_REG_LOW_R_LIMIT;
    MS_U16 DEMURA_CTRL_REG_PLANE_LEVEL[SIO_DEMURA_MAX_PLANE];
    MS_U16 DEMURA_CTRL_REG_HIGH_R_LIMIT;
