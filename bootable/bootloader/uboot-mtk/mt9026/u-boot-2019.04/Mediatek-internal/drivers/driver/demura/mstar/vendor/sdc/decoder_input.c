...
#define SAMPLE_DAT_START    2*(481*271+1)     // Byte
...
static MS_BOOL Gen_Sf_Signature(SDC_Demura_Header *phdr)
{
    char   strbuf[1024];
    MS_U32 sample_crc32;
    MS_U8  *buffer;
...
static MS_BOOL SDC_Decode_To_Mstar_Format(SDC_Demura_Header *phdr, interface_info *pinfo)
{
    ...
    for(k = (MAX_PLANE_NUM - 1); k >= 0; k--)//test for(k = (pinfo->reg_dmc_plane_num - 1); k >= 0; k--)
    {
        for (i = 0; i < phdr->DEMURA_TBL_V; i++)
        {
            ...
            {
                // Update Value_1
                Value_1 = (1<<phdr->Value_1[k])*(255.0/4096.0);//(phdr->Value_1[k] == 0)?255.0/4096.0:255.0/2048.0;
...
MS_BOOL Decode_To_Mstar_Format(void *pdat_info, BinOutputInfo *pbin_info)
{
    MS_BOOL bRet;
    ...
    #if(FORCE_TO_4_LAYER)
    pDataInfo->reg_dmc_plane_num              = 4;//test MAX_PLANE_NUM;         //  4 bit
    #else
    pDataInfo->reg_dmc_plane_num              = MAX_PLANE_NUM;
    #endif
    ...
    pbin_info->LevelCount     = MAX_PLANE_NUM;//test pDataInfo->reg_dmc_plane_num;
    ...
    #if(FORCE_TO_4_LAYER)
    pDataInfo->reg_dmc_plane_level1           = header.PLANE[SDC_Select_Layer[0]-1]<<4;       // 12 bit , format 10.2
    pDataInfo->reg_dmc_plane_level2           = header.PLANE[SDC_Select_Layer[1]-1]<<4;       // 12 bit , format 10.2
    pDataInfo->reg_dmc_plane_level3           = header.PLANE[SDC_Select_Layer[2]-1]<<4;       // 12 bit , format 10.2
    pDataInfo->reg_dmc_plane_level4           = header.PLANE[SDC_Select_Layer[3]-1]<<4;       // 12 bit , format 10.2
