...
static MS_BOOL _MDrv_GOP_SC_Set_ScalingFac(VOID)
{
    ...
    DBG_GOP_SC(printf("[GOP_SC] VdeIn = %lx \t H=%lx\n", (unsigned long)gGopSC_Info.InCfg.u16VdeIn, (unsigned long)gGopSC_Ref.u16ValidH);)
    DBG_GOP_SC(printf("[GOP_SC] ValidV = %lx\t HdeIn=%lx\n", (unsigned long)gGopSC_Ref.u16ValidV, (unsigned long)gGopSC_Info.InCfg.u16HdeIn);)
