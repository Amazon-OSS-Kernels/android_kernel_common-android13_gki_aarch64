...
static MS_BOOL _bEfuseInit = FALSE;
...
unsigned int _CheckOSDPQ2EnaByEfuse(void)
{
    MS_U32 ChipMajor = (u32ChipVersion & GOP_CAPS_MAJOR_IPVERSION_OFFSET_MSK) >> GOP_CAPS_MAJOR_IPVERSION_OFFSET_SHT;
    unsigned int efuseSupportOSDPQ = 1;

    if (ChipMajor >= GOP_MT5873_SERIES_MAJOR) {
        if (_bEfuseInit == FALSE) {
            if (mtk_efuse_check_IP(E_EFUSE_IDX_126, &efuseSupportOSDPQ)) {
                GOP_H_ERR("Error: get OSD PQ efuse fail\n");
            }
            _bEfuseInit = TRUE;
        }
    }

    return efuseSupportOSDPQ;
}
...
void HAL_GOP_Init(GOP_CTX_HAL_LOCAL *pGOPHalLocal, MS_U8 u8GOPNum)
{
    ...
    if (_CheckOSDPQ2EnaByEfuse() == 0)
        HAL_GOP_Write2bytemask(REG_0004_GOPG_BKA4D9, REG_0004_GOPG_BKA4D9_REG_ABF_BYPASS, REG_0004_GOPG_BKA4D9_REG_ABF_BYPASS);
}
...
GOP_Result HAL_GOP_SetGopStretchWin(GOP_CTX_HAL_LOCAL *pstGOPHalLocal, MS_U8 u8GOP, MS_U16 u16x, MS_U16 u16y, MS_U16 u16width, MS_U16 u16height)
{
    ...
    if (ChipMajor == GOP_MT5873_SERIES_MAJOR && ChipMinor == GOP_MT5873_SERIES_MINOR)
    {
        if (u8GOP == E_GOP1) {
