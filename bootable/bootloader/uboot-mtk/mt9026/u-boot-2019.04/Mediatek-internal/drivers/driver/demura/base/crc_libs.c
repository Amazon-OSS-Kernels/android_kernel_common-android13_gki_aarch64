...
MS_U32 hw_crc32_04C11DB7(MS_U8* pdat, MS_U32 dat_len)
{
    ...
    if(src_addr < /*CONFIG_SYS_MIU_INTERVAL*/0x80000000)
    {
        src_addr = src_addr;    // MIU0
        eDev = E_BDMA_SRCDEV_MIU0;
    }
    else if(src_addr < /*CONFIG_SYS_MIU2_INTERVAL*/0xC0000000)
    {
        src_addr = src_addr - /*CONFIG_SYS_MIU_INTERVAL*/0x80000000;    // MIU1
        eDev = E_BDMA_SRCDEV_MIU1;
    }
    else
    {
        src_addr = src_addr - /*CONFIG_SYS_MIU2_INTERVAL*/0xC0000000;    // MIU2
        eDev = E_BDMA_SRCDEV_MIU2;
    }
    UBOOT_DEBUG("eDev = %d\n", eDev);

    //MsOS_Dcache_Flush((MS_VIRT)pdat, dat_len);
     flush_dcache_range((unsigned long)pdat,( unsigned long)(pdat+dat_len));
