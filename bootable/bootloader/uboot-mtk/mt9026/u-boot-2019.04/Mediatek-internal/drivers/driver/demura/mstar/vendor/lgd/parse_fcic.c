...
//---------------------------------------------------------------
void CUC_cvt_blk_size_add_CIC(int ****data)
{
    ...
    for (t = 0; t < 4; t++)
    {
        for (c = 0; c < 3; c++)
        {
            ...
            for (j = 0; j < 69; j++)
            {
                int tmp[temp_size1] = { 0 };
                for (i = 0; i < temp_size1; i++)
                {
                    int dist_x_0 = (i % div);
                    int dist_x_1 = div - dist_x_0;
                    tmp[i] = (data[j][i / div][c][t] * dist_x_1 + data[j][minmax(i / div + 1, 0, 120)][c][t] * dist_x_0 + (div>>1)) / div;
                }
                for (i = 0; i < 481; i++)
                {
                    data[j][i][c][t] = tmp[i];
            ...
            for (i = 0; i < temp_size1; i++)
            {
                int tmp[temp_size2] = { 0 };
                for (j = 0; j < temp_size2; j++)
                {
                    int dist_y_0 = (j % div);
                    int dist_y_1 = div - dist_y_0;
                    tmp[j] = (data[j / div][i][c][t] * dist_y_1 + data[minmax(j / div + 1, 0, 67)][i][c][t] * dist_y_0 + (div)>>1) / div;
                }
                for (j = 0; j < temp_size2; j++)
                {
...
MS_BOOL parse_LG_bin_1tab(int n_Hnode, int n_Vnode, interface_info *info, MS_BOOL bFCIC)
{
    ...
    while (end_flag != TRUE)
    {
        ...
        for (i = 0; i < 32; i++) // Parse 32 bytes in one time
        {
            ...
            /* if (byte_address + i >= checksum_addr)
            {
            ...
            #if defined (CONFIG_DEMURA_VENDOR_LGD_V19) || defined (CONFIG_DEMURA_VENDOR_LGD_SQ18_19)
                ...
                else if (byte_address + i < chroma_4tab_addr) // luma (3:W, 1:Gray64)
                {
                    ...
                    #if defined (CONFIG_DEMURA_VENDOR_LGD_V19)
                    if (h_cnt == 252)
                    #elseif defined (CONFIG_DEMURA_VENDOR_LGD_SQ18_19)
                    if (h_cnt == 492)
                    #endif
                    {
                        h_cnt = 0;
                ...
                #if defined (CONFIG_DEMURA_VENDOR_LGD_V19)
                else if (byte_address + i < gain_offset_addr) // chroma (0~2:RGB)
                #elseif defined (CONFIG_DEMURA_VENDOR_LGD_SQ18_19)
   ...
   /*  for (t = 0; t < 4; t++)
    ...
    //            ->    comp_W = comp_Luma
    ///////////////////////////////////////////////////////////////////////////////////////
    #if defined (CONFIG_DEMURA_VENDOR_LGD_CIC_CUC)
        printf("CONFIG_DEMURA_VENDOR_LGD_CIC_CUC\n");

        CUC_cvt_blk_size_add_CIC(data);
    #else
