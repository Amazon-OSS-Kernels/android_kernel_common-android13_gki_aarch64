		      ...
		      disk_partition_t *info)
{
	...
	info->system = gpt_pte[part - 1].attributes.fields.required_to_function;
	info->hideefi = gpt_pte[part - 1].attributes.fields.no_block_io_protocol;
	info->ro = gpt_pte[part - 1].attributes.fields.type_guid_specific_read_only;
	info->hide = gpt_pte[part - 1].attributes.fields.type_guid_specific_hidden;
	info->nomount = gpt_pte[part - 1].attributes.fields.type_guid_specific_not_mount;
		 ...
		 disk_partition_t *partitions, int parts)
{
	...
	for (i = 0; i < parts; i++) {
		...
		if (partitions[i].system)
			gpt_e[i].attributes.fields.required_to_function = 1;

		if (partitions[i].hideefi)
			gpt_e[i].attributes.fields.no_block_io_protocol = 1;

		if (partitions[i].bootable)
			gpt_e[i].attributes.fields.legacy_bios_bootable = 1;

		if (partitions[i].ro)
			gpt_e[i].attributes.fields.type_guid_specific_read_only = 1;

		if (partitions[i].hide)
			gpt_e[i].attributes.fields.type_guid_specific_hidden = 1;

		if (partitions[i].nomount)
			gpt_e[i].attributes.fields.type_guid_specific_not_mount = 1;
...
#if defined(CONFIG_CUSTOMIZED_FIRST_PARTITION_OFFSET)
	if(dev_desc->if_type == IF_TYPE_USB)
		gpt_h->first_usable_lba = cpu_to_le64(0x100000 / dev_desc->blksz);  // USB first_usable_lba hard-code at 1MB, sync with ROM code
	else
		gpt_h->first_usable_lba =
			cpu_to_le64(CONFIG_FIRST_PARTITION_OFFSET / dev_desc->blksz);  // CONFIG_FIRST_PARTITION_OFFSET (in byte) / 512 (one lba size) = first_usable_lba
#else
	gpt_h->first_usable_lba =
		cpu_to_le64(le64_to_cpu(gpt_h->partition_entry_lba) + 32);        // default at lba 34 originally
#endif
