...
sbvc_result sboot_version_check(char *sboot_buf, int sboot_len, int partition_num)
{
	...
	const unsigned short version_bump = 0;
#elif defined(SBOOT_GINZA_BOARD)
	const char *device_name = "GINZA";
...
#elif defined(SBOOT_GINZALITE_BOARD)
...
/*	if ((sboot_ver >= 0x100) && (sboot_ver_dev < 0x100 )){
	        printf("\n%s: sboot img version: %d is secure image, dev version: %d  is non-secure SoC do not flash!!!\n",
		__FUNCTION__, sboot_ver, sboot_ver_dev);
		return SBVC_NON_SECURE;
	}
*/
	if ((sboot_ver < 0x100) && (sboot_ver_dev >= 0x100 )){
	        printf("\n%s: sboot img version: %d is non-secure image, dev version: %d  is Secure SoC , do not flash!!!\n",
		__FUNCTION__, sboot_ver, sboot_ver_dev);
		return SBVC_NON_SECURE;
	}
