...
typedef enum {
  ...
  //MTK code starts
  AVB_IO_RESULT_ERROR_RPMBKEYNOTEXISTS,
  //MTK code ends
} AvbIOResult;
...
struct AvbOps {
  ...
  //MTK code starts
  AvbIOResult (*write_device_status)(AvbOps *ops,
                                     uint16_t value);

  AvbIOResult (*write_vbmeta_data)(AvbOps* ops,
                                     const void* buffer,
                                     size_t size);
  AvbIOResult (*read_vbmeta_data)(AvbOps* ops,
                                     void* buffer,
                                     size_t size);
  //MTK code ends
