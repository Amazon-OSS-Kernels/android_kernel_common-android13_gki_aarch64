...
#include "avb_sha.h"
//MTK define start
#if defined(CONFIG_ARMV8_CE_SHA256)
...
#if defined(CONFIG_ARMV8_CE_SHA256)
//Use NEON acceleration to compute SHA256
  sha256_context context;

  memcpy(context.total, &ctx->tot_len, sizeof(context.total));
  memcpy(context.state, &ctx->h, sizeof(context.state));
  memcpy(context.buffer, &ctx->block, sizeof(context.buffer));

  sha256_starts(&context);
  memcpy(&ctx->tot_len, context.total, sizeof(context.total));
  memcpy(&ctx->h, context.state, sizeof(context.state));
  memcpy(&ctx->block, context.buffer, sizeof(context.buffer));
#else
#ifndef UNROLL_LOOPS
  int i;
  for (i = 0; i < 8; i++) {
