...
#ifdef CONFIG_AMAZON_UBOOT_SMP_OPTIMIZATION
typedef struct AmazonSmpOptimizationPartition {
	char part_name[15];
	volatile bool is_started_loading;
	volatile bool is_finished_loading;
	uint8_t* image_buf;
} AVB_ATTR_PACKED AmazonSmpOptimizationPartition;


#define AMAZON_SMP_OPTIMIZATION_PARTITION__ARRAY_SIZE  3
AmazonSmpOptimizationPartition g_AmazonSmpOptimizationPatition[AMAZON_SMP_OPTIMIZATION_PARTITION__ARRAY_SIZE] = {
	{"boot", false ,false, NULL},
	{"dtbo", false, false, NULL},
	{"vendor_boot", false, false, NULL}
};
bool amazon_smp_is_index_valid(int index)
{
	bool ret = false;
	if ((index >= 0) && (index  < AMAZON_SMP_OPTIMIZATION_PARTITION__ARRAY_SIZE))
	{
		ret = true;
	} else
	{
		printf("amazon: cpuid=%d, %s, %d, index = %d, this is invalid!!!\n", get_cpu_id(), __func__, __LINE__, index);
	}
	return ret;
}
int amazon_get_smp_optimization_partition_index(const char* part_name, int part_name_len)
{
	int i  = 0;
	int index = -1;

	for( i = 0; i < AMAZON_SMP_OPTIMIZATION_PARTITION__ARRAY_SIZE; i++)	{
		if(!avb_safe_memcmp(g_AmazonSmpOptimizationPatition[i].part_name, part_name, part_name_len)) {
			index = i;
			break;
		}
	}
...
bool  amazon_should_load_partition_right_now(int index)
{
	bool ret = false;

	if (amazon_smp_is_index_valid(index)) {
		if (g_AmazonSmpOptimizationPatition[index].is_started_loading == false)
		{
			ret = true;
		}
	}
...
AvbSlotVerifyResult amazon_smp_get_target_image_buff(int index, uint8_t** image_buf)
{
	AvbSlotVerifyResult ret = AVB_SLOT_VERIFY_RESULT_OK;

	char * part_name = g_AmazonSmpOptimizationPatition[index].part_name;
	*image_buf = NULL;

	if (amazon_smp_is_index_valid(index))
	{
		if(!avb_safe_memcmp(part_name, DTBO_PARTNAME, strlen(DTBO_PARTNAME))) {
			*image_buf = (uint8_t*)getDtboAddr();
		}
		else {
			const char* bootcmd = UBOOT_BOOTCMD;
			if(is_recovery_mode())
			{
				printf("amazon: cpuid = %d, %s, %d, index = %d, recovery mode\n", get_cpu_id(), __func__, __LINE__, index);
				bootcmd = UBOOT_REVOCERYCMD;
			}
			*image_buf = (uint8_t*)getImageAddr((const char* )part_name, bootcmd);
		}
		if(!(*image_buf)) {
			printf("amazon: cpuid = %d, %s, %d, Parsing %s from bootcmd fails\n", get_cpu_id(), __func__, __LINE__, part_name);
			ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_ARGUMENT;
			goto out;
		}
	} else {
		printf("amazon: cpuid = %d, %s, %d,index(%d) is invalid!\n", get_cpu_id(), __func__, __LINE__, index);
		ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_ARGUMENT;
	}
out:
...
bool amazon_smp_set_image_buf(int index, uint8_t *image_buf)
{
	bool ret = false;
	if (amazon_smp_is_index_valid(index))
	{
		g_AmazonSmpOptimizationPatition[index].image_buf = image_buf;
		ret = true;
	}
	return ret;
}

bool amazon_smp_get_image_buf(int index, uint8_t **image_buf)
{
	bool ret = false;
	if (amazon_smp_is_index_valid(index))
	{
		*image_buf = g_AmazonSmpOptimizationPatition[index].image_buf;
		ret = true;
	}
	return ret;
}

bool amazon_smp_set_targeted_partition_load_start(int index)
{
	bool ret = false;
	if (amazon_smp_is_index_valid(index))
	{
		g_AmazonSmpOptimizationPatition[index].is_started_loading = true;
		ret = true;
	}
	return ret;
}

bool amazon_smp_set_targeted_partition_load_finish(int index)
{
	bool ret = false;
	if (amazon_smp_is_index_valid(index))
	{
		g_AmazonSmpOptimizationPatition[index].is_finished_loading = true;
		ret = true;
	}
	return ret;
}

void amazon_wait_until_smp_partition_load_finished(int index)
{
	if (amazon_smp_is_index_valid(index)) {
		if (g_AmazonSmpOptimizationPatition[index].is_started_loading == true) {
			while (g_AmazonSmpOptimizationPatition[index].is_finished_loading != true){
...
AvbSlotVerifyResult amazon_smp_load_target_partition(AvbOps* ops,
										const char* part_name,
										int part_name_len,
										uint64_t image_size,
										uint8_t **image_buf,
										bool * should_try_original_load)
{
	AvbSlotVerifyResult ret = AVB_SLOT_VERIFY_RESULT_OK;
	int index = -1;

	*should_try_original_load = false;

	//1.Get index according part_name
	index = amazon_get_smp_optimization_partition_index(part_name, part_name_len);
#if 1
	if (index == -1){
		//If it's not targeted partition, exit directly!
		printf("zzl: cpuid=%d, %s, %d, part_name = %s,part_name_len = %d, index = -1, after called amazon_get_smp_optimization_partition_index(), there is no such partition!!!\n", get_cpu_id(), __func__, __LINE__, part_name, part_name_len);
		ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_ARGUMENT;
		*should_try_original_load = true;
		goto out;
	}

	//index is valid!
	//Now it's targeted partition!
	//2. if not loaded before, load now!
	if (amazon_should_load_partition_right_now(index) == true)
	{
		bool image_preloaded = false;

		//2.0 set start loading flag!
		amazon_smp_set_targeted_partition_load_start(index);

		//2.1 allocate image_buf
		ret = amazon_smp_get_target_image_buff(index, image_buf);
		if (ret != AVB_SLOT_VERIFY_RESULT_OK) {
			printf("amazon: cpuid=%d, %s, %d, part_name = %s,part_name_len = %d\n", get_cpu_id(), __func__, __LINE__, part_name, part_name_len);
			goto fail;
		}
		//2.2 load_partition
		ret = load_full_partition(ops, part_name, image_size, image_buf, &image_preloaded);
		if (ret != AVB_SLOT_VERIFY_RESULT_OK) {
			printf("amazon: cpuid=%d, %s, %d, part_name = %s,part_name_len = %d\n", get_cpu_id(), __func__, __LINE__, part_name, part_name_len);
			goto fail;
		}
		//2.3 set image_buf to global array!
		amazon_smp_set_image_buf(index, *image_buf);
fail:
		//even fail, we should set load finish flash!!!
		//2.4 set g_AmazonSmpOptimizationPatition[index].is_finished_loading to true!!!
		amazon_smp_set_targeted_partition_load_finish(index);
	} else {
	//3. already started loading, now wait untile target partition loading finished!
		amazon_wait_until_smp_partition_load_finished(index);
		amazon_smp_get_image_buf(index,  image_buf);
	}
#endif
out:
...
AvbSlotVerifyResult amazon_get_image_hash_size(AvbOps* ops,
										...
										uint64_t *image_size)
{
	...
	if (!avb_hash_descriptor_validate_and_byteswap(
	    ...
	    printf("amazon:cpuid = %d, %s, %d, partition_name = %s, ret =  AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA\n", get_cpu_id(), __func__, __LINE__, partition_name);
...
AvbSlotVerifyResult amazon_get_footer_image_size(AvbOps* ops,
												const char* partition_name,
												...
												uint64_t *image_size)
{
	...
	if (vbmeta_buf == NULL) {
		printf("amazon: cpuid = %d,%s, %d    partition_name = %s, vbmeta_buf == NULL, AVB_SLOT_VERIFY_RESULT_ERROR_OOM\n", get_cpu_id(), __func__, __LINE__,partition_name);
		ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
		goto out;
	}
	//1.get vbmeta_buf, using vbmeta_offsize,
	ret = ops->read_from_partition(ops,
								...
								vbmeta_buf,
								&vbmeta_num_read);
	if (ret == AVB_SLOT_VERIFY_RESULT_ERROR_OOM) {
		printf("amazon: cpuid = %d,%s, %d    partition_name = %s, AVB_SLOT_VERIFY_RESULT_ERROR_OOM\n", get_cpu_id(), __func__, __LINE__,partition_name);
		ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
		goto out;
	}
	...
	//2.through vbmeta_buf, get descriptors[]
    descriptors = avb_descriptor_get_all(vbmeta_buf, vbmeta_num_read, &num_descriptors);
    if (descriptors == NULL) {
		printf("amazon: cpuid = %d,%s, %d    partition_name = %s, descriptors == NULL\n", get_cpu_id(), __func__, __LINE__,partition_name);
        ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
        goto out;
    }

	ret = amazon_get_image_hash_size(ops, partition_name, descriptors[0], image_size);
out:
	if (vbmeta_buf != NULL)
	{
		avb_free(vbmeta_buf);
...
AvbSlotVerifyResult amazon_get_footer_info(AvbOps* ops,
										const char* partition_name,
										size_t *vbmeta_offset,
										size_t * vbmeta_size)
{
	AvbSlotVerifyResult ret = AVB_SLOT_VERIFY_RESULT_OK;
	AvbIOResult io_ret;
	size_t tmp_vbmeta_offset = 0;
    size_t tmp_vbmeta_size = VBMETA_MAX_SIZE;
	uint8_t tmp_footer_buf[AVB_FOOTER_SIZE];
    size_t tmp_footer_num_read;
    AvbFooter footer;
	io_ret = ops->read_from_partition(ops,
                                     "boot",
                                     -AVB_FOOTER_SIZE,
                                     AVB_FOOTER_SIZE,
                                      ...
                                      &tmp_footer_num_read);
	if (io_ret == AVB_IO_RESULT_ERROR_OOM) {
		printf("amazon: cpuid = %d, %s, %d partition_name = %s after calling read_from_partition, ret = %d, AVB_SLOT_VERIFY_RESULT_ERROR_OOM goto out\n", get_cpu_id(), __func__, __LINE__, partition_name, ret);
		ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
		goto out;
	} else if (io_ret != AVB_IO_RESULT_OK) {
	  printf("amazon: cpuid = %d, %s, %d partition_name = %s after calling read_from_partition, ret = %d, Error loading footer goto out\n", get_cpu_id(), __func__, __LINE__, partition_name, ret);
	...
	avb_assert(footer_num_read == AVB_FOOTER_SIZE);
	 ...
	 if (!avb_footer_validate_and_byteswap((const AvbFooter*)tmp_footer_buf,
                                          &footer)) {
      printf("amazon: cpuid = %d, %s, %d, No footer detected.\n", get_cpu_id(), __func__, __LINE__);
    } else {
      /* Basic footer sanity check since the data is untrusted. */
      if (footer.vbmeta_size > VBMETA_MAX_SIZE) {
		printf("amazon: cpuid = %d, %s, %d, Invalid vbmeta size in footer.\n", get_cpu_id(), __func__, __LINE__);
      } else {
        tmp_vbmeta_offset = footer.vbmeta_offset;
        tmp_vbmeta_size = footer.vbmeta_size;
      }
    }
out:
	*vbmeta_offset = tmp_vbmeta_offset;
	*vbmeta_size = tmp_vbmeta_size;
	return ret;
}

AvbSlotVerifyResult amazon_preload_single_chain_image(AvbOps* ops,
													...
													const char* partition_name)
{
	...
	if (allow_verification_error) {
		//printf("amazon: cpuid = %d, %s, %d allow_verification_error = %d, partition_name = %s\n", get_cpu_id(), __func__, __LINE__, allow_verification_error, partition_name);
		AvbIOResult io_ret;
		io_ret = ops->get_size_of_partition(ops, partition_name, &image_size);
		if (io_ret == AVB_IO_RESULT_ERROR_OOM) {
			printf("amazon: cpuid = %d, %s, %d allow_verification_error = %d, partition_name = %s, AVB_SLOT_VERIFY_RESULT_ERROR_OOM\n", get_cpu_id(), __func__, __LINE__, allow_verification_error, partition_name);
			ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
			goto out;
		} else if (io_ret != AVB_IO_RESULT_OK) {
			printf("amazon: cpuid = %d, %s, %d allow_verification_error = %d, partition_name = %s, rror determining partition size\n", get_cpu_id(), __func__, __LINE__, allow_verification_error, partition_name);
			ret = AVB_SLOT_VERIFY_RESULT_ERROR_IO;
			goto out;
		}
    } else {
		ret = amazon_get_footer_info(ops, partition_name, &vbmeta_offset, &vbmeta_size);
		if (ret != AVB_SLOT_VERIFY_RESULT_OK)
		{
			printf("amazon: cpuid = %d, %s, %d allow_verification_error = %d, partition_name = %s, call amazon_get_footer_info() failed  \n", get_cpu_id(), __func__, __LINE__, allow_verification_error, partition_name);
			goto out;
		}

		ret = amazon_get_footer_image_size(ops, partition_name, vbmeta_offset, vbmeta_size, &image_size);
		if (ret != AVB_SLOT_VERIFY_RESULT_OK)
		{
			printf("amazon: cpuid = %d, %s, %d allow_verification_error = %d, partition_name = %s, call amazon_get_footer_image_size() failed\n", get_cpu_id(), __func__, __LINE__, allow_verification_error, partition_name);
			goto out;
		}
	}
	bool should_try_original_load = false;
	uint8_t* image_buf =  NULL;

	ret = amazon_smp_load_target_partition(ops, partition_name,  avb_strlen(partition_name), image_size, &image_buf, &should_try_original_load);
out:
	return ret;
}
static const char* const amazon_smp_optimized_hash_partitions[] = {"dtbo",
																	"vendor_boot",
																	NULL};

AvbSlotVerifyResult amazon_get_hash_partition_name(const AvbDescriptor* descriptor,  const char* ab_suffix, const char ** partition_name)
{
	AvbSlotVerifyResult ret = AVB_SLOT_VERIFY_RESULT_OK;
	AvbHashDescriptor hash_desc;
	const char* found;
	...
	if (!avb_hash_descriptor_validate_and_byteswap(
		(const AvbHashDescriptor*)descriptor, &hash_desc)) {
		printf("amazon: cpuid = %d, %s, %d , AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA\n", get_cpu_id(), __func__, __LINE__);
		ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
		goto out;
    }
	desc_partition_name = ((const uint8_t*)descriptor) + sizeof(AvbHashDescriptor);

	found = avb_strv_find_str(amazon_smp_optimized_hash_partitions,
                              (const char*)desc_partition_name,
                              hash_desc.partition_name_len);
    if (found == NULL) {
		printf("amazon: cpuid = %d, %s, %d , didn't found the proper partitioin name!!!\n", get_cpu_id(), __func__, __LINE__);
		ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_ARGUMENT;
		goto out;
    }
...
AvbSlotVerifyResult amazon_get_vbmeta_descriptors_and_load_all_hash_images(AvbOps* ops, bool allow_verification_error, const char* ab_suffix)
{
	AvbSlotVerifyResult ret = AVB_SLOT_VERIFY_RESULT_OK;
	size_t vbmeta_offset;
	size_t vbmeta_size;
	...
	if (allow_verification_error)
	{
		const char * const* p_amazon_hash_partition = amazon_smp_optimized_hash_partitions;
		 for (n = 0; p_amazon_hash_partition[n] != NULL; n++)
		{
			uint64_t image_size = 0;
			avb_memset(full_partition_name, '\0', AVB_PART_NAME_MAX_SIZE);
			if (!avb_str_concat(full_partition_name,
							sizeof full_partition_name,
							p_amazon_hash_partition[n],
							avb_strlen(p_amazon_hash_partition[n]),
							ab_suffix,
							avb_strlen(ab_suffix))) {
				printf("amazon: cpuid = %d, %s, %d, allow_verification_error = %d,  full_partition_name = %s \n", get_cpu_id(), __func__, __LINE__, allow_verification_error, full_partition_name);
			...
			if (io_ret == AVB_IO_RESULT_ERROR_OOM) {
				printf("amazon: cpuid=%d, %s, %d, allow_verification_error , full_partition_name = %s, new image_size=%lld, AVB_IO_RESULT_ERROR_OOM\n", get_cpu_id(),  __func__, __LINE__, full_partition_name, image_size);
				ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
				goto out;
			} else if (io_ret != AVB_IO_RESULT_OK) {
				printf("amazon: cpuid=%d, %s, %d, allow_verification_error , full_partition_name = %s, new image_size=%lld, Error determining partition size\n", get_cpu_id(),  __func__, __LINE__, full_partition_name, image_size);
				ret = AVB_SLOT_VERIFY_RESULT_ERROR_IO;
				goto out;
			}
		}
		...
		if (!avb_str_concat(full_partition_name,
							...
							ab_suffix,
							avb_strlen(ab_suffix))) {
			printf("amazon: cpuid = %d, %s, %d partition_name = \"vbmeta\", artition name and suffix does not fit\n", get_cpu_id(), __func__, __LINE__);
		...
		//1. Get vbmeta partitioin buffer!####################################
		io_ret = ops->read_from_partition(ops,
										  full_partition_name,
										  vbmeta_offset,
										  vbmeta_size,
		...
		if (io_ret == AVB_IO_RESULT_ERROR_OOM) {
			printf("amazon: cpuid = %d, %s, %d full_partition_name = \"%s\", AVB_SLOT_VERIFY_RESULT_ERROR_OOM\n", get_cpu_id(), __func__, __LINE__,  full_partition_name);
			ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
			goto out;
		} else if (io_ret != AVB_IO_RESULT_OK) {
			printf("amazon: cpuid = %d, %s, %d full_partition_name = \"%s\", Error loading vbmeta data\n", get_cpu_id(), __func__, __LINE__,  full_partition_name);
			ret = AVB_SLOT_VERIFY_RESULT_ERROR_IO;
			goto out;
		...
		//3. Read all descriptoers and load hash partitions 
		for (n = 0; n < num_descriptors; n++) {
			AvbDescriptor desc;

			if (!avb_descriptor_validate_and_byteswap(descriptors[n], &desc)) {
				printf("amazon: cpuid = %d, %s, %d full_partition_name = \"%s\", Descriptor is invalid\n", get_cpu_id(), __func__, __LINE__,  full_partition_name);
				ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
				goto out;
			}
			switch (desc.tag) {
				case AVB_DESCRIPTOR_TAG_HASH:{
					...
					uint64_t image_size = 0;
					//3.1 Judge ,whether current descriptors[n] is wanted one!
					ret = amazon_get_hash_partition_name(descriptors[n],  ab_suffix, &partition_name);
					if (ret != AVB_SLOT_VERIFY_RESULT_OK) //didn't found parper partition name
						continue;
					if (partition_name == NULL)
					{
						//printf("amazon: cpuid = %d, %s, %d full_partition_name = \"%s\",  n = %ld, num_descriptors = %ld, not the wanted partition name, so skip!\n", get_cpu_id(), __func__, __LINE__,  full_partition_name, n, num_descriptors);
						break;
					}

					//3.2 Load the partitions of the descriptors[n]
					ret = amazon_get_image_hash_size(ops, partition_name, descriptors[n], &image_size);
					if (ret != AVB_SLOT_VERIFY_RESULT_OK)
					{
						printf("amazon: cpuid = %d, %s, %d, full_partition_name =%s, n = %ld, num_descriptors = %ld, found the wanted partition_name = \"%s\", AVB_SLOT_VERIFY_RESULT_ERROR_OOM\n", get_cpu_id(), __func__, __LINE__,  full_partition_name, n, num_descriptors, partition_name);
						ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
						goto out;
					}
...
AvbSlotVerifyResult amazon_preload_image(AvbOps* ops, bool allow_verification_error, const char* ab_suffix)
{
	AvbSlotVerifyResult ret = AVB_SLOT_VERIFY_RESULT_OK;

	ret = amazon_preload_single_chain_image(ops, allow_verification_error, "boot");

	if (ret != AVB_SLOT_VERIFY_RESULT_OK)
	{
		printf("amazon: cpuid = %d, %s, %d partition_name = \"boot\", call amazon_preload_single_image() failed\n", get_cpu_id(), __func__, __LINE__);
		goto out;
	}
	ret = amazon_get_vbmeta_descriptors_and_load_all_hash_images(ops, allow_verification_error, ab_suffix);

	if (ret != AVB_SLOT_VERIFY_RESULT_OK)
	{
		printf("amazon: cpuid = %d, %s, %d partition_name = \"dtbo\", call amazon_preload_single_image() failed\n", get_cpu_id(), __func__, __LINE__);
		goto out;
	}
...
//From here, is the smp thread related!
volatile bool g_amazon_smp_thread_can_run = false;
bool g_allow_verification_error = false;
const char* g_slot_suffix = NULL;
AvbOps* g_ops = NULL;

void amazon_thread_wait_until_run(void)
{
	while(g_amazon_smp_thread_can_run != true)
	{

	}
}

void amazon_init_smp_thread_signal(void)
{
	g_amazon_smp_thread_can_run = false;
}
void amazon_notify_smp_thread_to_run(void)
{
	g_amazon_smp_thread_can_run = true;
}

void amazon_smp_thread_prepare_param(AvbOps* ops, bool allow_verification_error, const char* ab_suffix)
{
	g_ops = ops;
	g_allow_verification_error = allow_verification_error;
	g_slot_suffix = ab_suffix;

}
void *amazon_thread_preload_boot(void *arg)
{
	AvbSlotVerifyResult ret = AVB_SLOT_VERIFY_RESULT_OK;

	amazon_thread_wait_until_run();

	ret = amazon_preload_single_chain_image(g_ops, g_allow_verification_error, "boot");
	if (ret != AVB_SLOT_VERIFY_RESULT_OK)
	{
		printf("amazon: cpuid = %d, %s, %d partition_name = \"boot\", call amazon_preload_single_image() failed\n", get_cpu_id(), __func__, __LINE__);
	}
...
void *amazon_thread_preload_others(void *arg)
{
	AvbSlotVerifyResult ret = AVB_SLOT_VERIFY_RESULT_OK;

	amazon_thread_wait_until_run();

	ret = amazon_get_vbmeta_descriptors_and_load_all_hash_images(g_ops, g_allow_verification_error, g_slot_suffix);
...
thread_t *avb_preload_boot_thread_id = NULL;
thread_t *avb_preload_others_thread_id = NULL;

void amazon_init_optimize(void)
{
#if (CONFIG_CMD_AVB == 1)
	run_command("avb init 0", 0);

	amazon_init_smp_thread_signal();

	avb_preload_boot_thread_id = thread_create_ex("preload_boot", amazon_thread_preload_boot, NULL, 0, MUTLI_CORE_CPU_ONE, THREAD_MAX_PRIORITY, 0);
    if (avb_preload_boot_thread_id == NULL )
    {
		printf("amazon:cpuid=%d, %s, %d create thread \"preload_boot\" failed!!!\n", get_cpu_id(), __func__, __LINE__);
    }
	avb_preload_others_thread_id = thread_create_ex("preload_others", amazon_thread_preload_others, NULL, 0, MUTLI_CORE_CPU_TWO, THREAD_MAX_PRIORITY, 0);
    if (avb_preload_others_thread_id == NULL )
    {
		printf("amazon:cpuid=%d, %s, %d create thread \"preload_boot\" failed!!!\n", get_cpu_id(), __func__, __LINE__);
    }
...
static AvbSlotVerifyResult load_and_verify_hash_partition(
...
#ifdef CONFIG_AMAZON_UBOOT_SMP_OPTIMIZATION
	bool should_try_original_load = false;
	ret = amazon_smp_load_target_partition(ops, part_name,  avb_strlen(part_name), image_size, &image_buf, &should_try_original_load);

	//1.0 in this case, that amazon already the image load!
	if (should_try_original_load == false)
		goto amazon_continue;

	//2.0 else, the part_name is not in g_AmazonSmpOptimizationPatition, use original code logic!!!!
#endif

  //MTK code starts
  //In case partition is dtbo
  if(!avb_safe_memcmp(part_name, DTBO_PARTNAME, strlen(DTBO_PARTNAME))) {
    image_buf = (uint8_t*)getDtboAddr();
  }
  else {
  ...
  if(!image_buf) {
    UBOOT_ERROR("Parsing %s from bootcmd fails.\n", part_name);
    ret = AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_ARGUMENT;
    goto out;
  }
  //MTK code ends
...
amazon_continue:
  ...
  //MTK code starts
  if(!avb_safe_memcmp(part_name, DTBO_PARTNAME, strlen(DTBO_PARTNAME))) {
    //Raise isDTBO_Verified
    extern unsigned int b_isDTBO_Verified;
    b_isDTBO_Verified = true;
  }
  //MTK code ends
...
static AvbSlotVerifyResult load_and_verify_vbmeta(
...
#ifdef CONFIG_AMAZON_UBOOT_SMP_OPTIMIZATION
	amazon_smp_thread_prepare_param(ops, allow_verification_error, ab_suffix);
	amazon_notify_smp_thread_to_run();
#endif
  }
  ...
  vbmeta_image_data->public_key_data = (uint8_t*)pk_data;
  ...
  //MTK code starts
  //Version upgrade detected, store current version number
  if (vbmeta_header.rollback_index > stored_rollback_index) {
...
static bool has_system_partition(AvbOps* ops, const char* ab_suffix) {
  char part_name[AVB_PART_NAME_MAX_SIZE];
  char* system_part_name = SYSTEM_PARTNAME;
  char guid_buf[37];
  AvbIOResult io_ret;
...
AvbSlotVerifyResult avb_slot_verify(AvbOps* ops,
  ...
  //MTK code starts
  avb_assert(ops->write_device_status != NULL);
  //MTK code ends
  if (out_data != NULL) {
    *out_data = NULL;
  }
  ...
  if (!(flags & AVB_SLOT_VERIFY_FLAGS_NO_VBMETA_PARTITION)) {
    if (avb_strcmp(slot_data->vbmeta_images[0].partition_name, "vbmeta") != 0) {
      avb_assert(
          avb_strcmp(slot_data->vbmeta_images[0].partition_name, BOOT_PARTNAME) == 0);
...
//printf("zzl: cpuid=%d, %s, after avb_append_options\n", get_cpu_id(),  __func__);
    if (sub_ret != AVB_SLOT_VERIFY_RESULT_OK) {
      ret = sub_ret;
      goto fail;
    }
