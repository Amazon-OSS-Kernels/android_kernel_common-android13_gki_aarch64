...
int aes_get_trng_cmd(char *command)
{
	if (!strcmp(command, "aes.trng"))
		return 0;
	return -1;
}
...
void trng_test(void)
{
#define TRNG_TEST_BUFSIZE (0x10)
	unsigned char trng[TRNG_TEST_BUFSIZE] = {0};

	if (HW_TRNG_GetRandomNumer((unsigned char *)&trng[0], TRNG_TEST_BUFSIZE) < 0) {
		printf("HW_TRNG_GetRandomNumer fail!\n");
	} else {
		int i;

		printf("HW_TRNG_GetRandomNumer pass!\n");
		for (i = 0; i < TRNG_TEST_BUFSIZE; i++)
			printf("0x%x ", trng[i]);
	}
}
