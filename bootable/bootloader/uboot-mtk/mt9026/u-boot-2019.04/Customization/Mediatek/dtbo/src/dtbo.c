...
void model_name_factory_customized(const char **relpath)
{
	...
	if (strcmp(factory_name, "")) {
		if (!strcmp(factory_name, "hisense")) {
			if (snprintf(new_path, sizeof(new_path), "dtb/%s", CONFIG_VBYONE_DTBO_CFG_NAME) > sizeof(new_path)) {
...
#ifdef CONFIG_DATA_SEPARATION
#define MMAP_2G_STRING "_2g"
#define MMAP_1POINT5G_STRING "_1point5g"
#define MMAP_1G_STRING "_1g"
#endif
#endif

#define MT5873_PAD_BOND_BANK (0x100A)
#define MT5873_PAD_BOND_ADDR (0x0)
#define MT5873_PAD_HTC_SHIFT (0x4)
#define MT5873_PAD_HTC_MASK (0x1)
#define MT5873_PAD_PKG_SHIFT (0x8)
#define MT5873_PAD_PKG_MASK (0x1)
#define MT5873_BOND_NTC (0x0)
#define MT5873_BOND_HTC (0x1)
#define MT5873_PKG_SMALL (0x0)
#define MT5873_PKG_BIG (0x1)
...
static int mmap_auto_detection_callback(char* detected_name, int size)
{
    ...
    UBOOT_DEBUG("MMAP auto detection - AOSP\n");
    {
        ...
        else if (MT5873 == romtbl_get_chip_id_info())
        {
#ifdef AOSP_ONE_IMAGE_SUPPORT
            if (dram_size == 0x80000000ULL) //2GB
            {
                UBOOT_DEBUG("WARNING: DRAM size is 2GB\n");
#ifdef COMMERCIAL_DISPLAY_SUPPORT
                strncpy(detected_name, "mmap_mt5873_h1v1_an_2g_iommu_cd", size-1);
#else
                strncpy(detected_name, "mmap_mt5873_h1v1_an_2g_iommu", size-1);
#endif
                return 0;
            }
            else if (dram_size == 0x60000000ULL) //1p5GB
            {
                UBOOT_DEBUG("WARNING: DRAM size is 1.5GB\n");
                strncpy(detected_name, "mmap_mt5873_h1v1_an_1point5g_iommu", size-1);
                return 0;
            }
            else if (dram_size == 0x40000000ULL) //1GB
            {
                UBOOT_DEBUG("WARNING: DRAM size is 1GB\n");
                strncpy(detected_name, "mmap_mt5873_h1v1_an_1g_iommu", size-1);
                return 0;
            }
            else if (dram_size == 0x100000000ULL) //4GB
            {
#ifdef COMMERCIAL_DISPLAY_SUPPORT
                strncpy(detected_name, "mmap_mt5873_h1v1_an_4g_iommu_cd", size-1);
#else
                strncpy(detected_name, "mmap_mt5873_h1v1_an_4g_iommu", size-1);
#endif
                return 0;
            }
            else if (dram_size == 0xC0000000ULL) //3GB
            {
#ifdef COMMERCIAL_DISPLAY_SUPPORT
                strncpy(detected_name, "mmap_mt5873_h1v1_an_3g_iommu_cd", size-1);
#else
                strncpy(detected_name, "mmap_mt5873_h1v1_an_3g_iommu", size-1);
#endif
                return 0;
            }
            else
                UBOOT_DEBUG("AOSP_ONE_IMAGE_SUPPORT is failed\n");
#else
            UBOOT_DEBUG("AOSP_ONE_IMAGE_SUPPORT is not supported\n");
#endif
        }
	...
	UBOOT_DEBUG("resv_mem auto detection - AOSP\n");
	{
		...
		if (MT5879 == romtbl_get_chip_id_info())
		{
			...
			UBOOT_DEBUG("AOSP_ONE_IMAGE_SUPPORT is not supported\n");
                ...
                {
#ifdef AOSP_ONE_IMAGE_SUPPORT
                        if (dram_size == SIZE_2G)
                        {
                                UBOOT_DEBUG("WARNING: DRAM size is 2GB\n");
                                strlcpy(detected_name, "resv_mem_mt5873_h1v1_an_2g_iommu", size-1);
                                return 0;
                        }
                        else if (dram_size == SIZE_1p5G)
                        {
                                UBOOT_DEBUG("WARNING: DRAM size is 1.5GB\n");
                                strlcpy(detected_name, "resv_mem_mt5873_h1v1_an_1point5g_iommu", size-1);
                                return 0;
                        }
                        else if (dram_size == SIZE_1G)
                        {
                                UBOOT_DEBUG("WARNING: DRAM size is 1GB\n");
                                strlcpy(detected_name, "resv_mem_mt5873_h1v1_an_1g_iommu", size-1);
                                return 0;
                        }
                        else if (dram_size == SIZE_4G)
                        {
                                UBOOT_DEBUG("WARNING: DRAM size is 4GB\n");
                                strlcpy(detected_name, "resv_mem_mt5873_h1v1_an_4g_iommu", size-1);
                                return 0;
                        }
                        else if (dram_size == SIZE_3G)
                        {
                                UBOOT_DEBUG("WARNING: DRAM size is 3GB\n");
                                strlcpy(detected_name, "resv_mem_mt5873_h1v1_an_3g_iommu", size-1);
                                return 0;
                        }
                        else
                                UBOOT_DEBUG("AOSP_ONE_IMAGE_SUPPORT is failed\n");
...
static int iommu_cust_auto_detection_callback(char* detected_name, int size)
{
	...
	switch (dram_size)
	{
		...
		case SIZE_2G:
			...
			break;
                case SIZE_1p5G:
                        len = snprintf(detected_name, size - 1, "mt%d_%s", chip_number, "1point5g");
                        break;
                case SIZE_1G:
                        len = snprintf(detected_name, size - 1, "mt%d_%s", chip_number, "1g");
                        break;
		default:
...
/*
 ...
 return otherwise: fail
*/
static int thermal_auto_detection_callback(char* detected_name, int size)
{
	int chip_id = -1;
	unsigned short bond = 0xFFFF;
	int pkg = 0, htc = 0;
	...
	{
		case MT5873:
			bond = CPU_REGISTER_ACCESS(MT5873_PAD_BOND_BANK, MT5873_PAD_BOND_ADDR);
			htc = ((bond >> MT5873_PAD_HTC_SHIFT) & MT5873_PAD_HTC_MASK);
			pkg = ((bond >> MT5873_PAD_PKG_SHIFT) & MT5873_PAD_PKG_MASK);

			if (htc == MT5873_BOND_NTC && pkg == MT5873_PKG_BIG)
				strncpy(detected_name, "thermal_mt5873_ntc", size-1);
			if (htc == MT5873_BOND_NTC && pkg == MT5873_PKG_SMALL)
				strncpy(detected_name, "thermal_mt5873_ntc_small", size-1);
			if (htc == MT5873_BOND_HTC && pkg == MT5873_PKG_BIG)
				strncpy(detected_name, "thermal_mt5873_htc", size-1);
			break;
		default:
			UBOOT_DEBUG("thermal Chip id %d is not support thermal\n", chip_id);
			return 1;
	}

	UBOOT_DEBUG("DEBUG: bound: %d, thermal select name: %s.dtso\n", bond, detected_name);
	return 0;
}
...
static int dtbo_init_auto_detection_callbacks(void)
{
	...
	if (dtbo_register_auto_detection_callback("thermal", thermal_auto_detection_callback))
		UBOOT_INFO("Warning: thermal register failed\n");
