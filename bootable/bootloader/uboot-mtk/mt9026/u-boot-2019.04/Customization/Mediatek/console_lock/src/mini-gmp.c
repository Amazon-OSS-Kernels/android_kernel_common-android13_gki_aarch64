...
#ifdef MINI_GMP_UBOOT_PORTING
#include <log.h>	// assert
#include <linux/ctype.h>
#include <linux/kernel.h>
#include <linux/bug.h> /* for BUG() */
#else
#include <assert.h>
#include <ctype.h>
#include <limits.h>
...
#ifdef MINI_GMP_UBOOT_PORTING
#define CHAR_BIT 8	// generic limit.h
#endif
...
#define gmp_umul_ppmm(w1, w0, u, v)					\
  do {									\
	...
	__x3 += GMP_HLIMB_BIT;	/* yes, add it in the proper pos. */	\
									\
      (w1) = __x3 + (__x1 >> (GMP_LIMB_BITS / E_INT_2));			\
      (w0) = (__x1 << (GMP_LIMB_BITS / E_INT_2)) + (__x0 & GMP_LLIMB_MASK);	\
    }									\
  } while (0)
...
mpn_invert_3by2 (mp_limb_t u1, mp_limb_t u0)
{
...

  {
    ...
    */
    ...
    r = ((~u1 - (mp_limb_t) qh * uh) << (GMP_LIMB_BITS / E_INT_2)) | GMP_LLIMB_MASK;
    ...
    p = (r >> (GMP_LIMB_BITS / E_INT_2)) * qh + r;
    /* Unlike full-limb 3/2, we can add 1 without overflow. For this to
       work, it is essential that ql is a full mp_limb_t. */
    ql = (p >> (GMP_LIMB_BITS / E_INT_2)) + 1;

    /* By the 3/2 trick, we don't need the high half limb. */
    r = (r << (GMP_LIMB_BITS / E_INT_2)) + GMP_LLIMB_MASK - ql * u1;

    if (r >= (GMP_LIMB_MAX & (p << (GMP_LIMB_BITS / E_INT_2))))
      {
	ql--;
	r += u1;
      }
    m = ((mp_limb_t) qh << (GMP_LIMB_BITS / E_INT_2)) + ql;
    if (r >= u1)
      {
	m++;
	r -= u1;
		   ...
		   mp_srcptr dp, mp_size_t dn)
{
  ...
  if (dn == 1)
	...
	{
	  d1 = (d1 << shift) | (d0 >> (GMP_LIMB_BITS - shift));
	  d0 = (d0 << shift) | (dp[dn-E_INT_3] >> (GMP_LIMB_BITS - shift));
	}
      inv->d1 = d1;
      inv->d0 = d0;
      inv->di = mpn_invert_3by2 (d1, d0);
		...
		mp_limb_t dinv)
{
  ...
  do
	...
	{
	  gmp_udiv_qr_3by2 (q, n1, n0, n1, n0, np[dn-E_INT_2+i], d1, d0, dinv);

	  cy = mpn_submul_1 (np + i, dp, dn-E_INT_2, q);
...
mpn_base_power_of_two_p (unsigned b)
{
  switch (b)
    {
    case E_2_POWER_1: return E_INT_1;
    case E_2_POWER_2: return E_INT_2;
    case E_2_POWER_3: return E_INT_3;
    case E_2_POWER_4: return E_INT_4;
    case E_2_POWER_5: return E_INT_5;
    case E_2_POWER_6: return E_INT_6;
    case E_2_POWER_7: return E_INT_7;
    case E_2_POWER_8: return E_INT_8;
    default: return E_INT_0;
    }
}
...
mpz_set_d (mpz_t r, double x)
{
  ...
  B = DOUBLE_4_0 * (double) (GMP_LIMB_HIGHBIT >> 1);
  Bi = 1.0 / B;
  for (rn = 1; x >= B; rn++)
    x *= Bi;
...
mpz_get_d (const mpz_t u)
{
  ...
  mp_size_t un;
  double x;
  double B = DOUBLE_4_0 * (double) (GMP_LIMB_HIGHBIT >> 1);
...
mpz_rootrem (mpz_t x, mpz_t r, const mpz_t y, unsigned long z)
{
  ...
  mpz_init (u);
  mpz_init (t);
  mpz_setbit (t, mpz_sizeinbase (y, E_INT_2) / z + 1);

  if (z == E_INT_2) /* simplify sqrt loop: z-1 == 1 */
    do {
      mpz_swap (u, t);			/* u = x */
...
{
  mpz_rootrem (s, r, u, E_INT_2);
}

void
mpz_sqrt (mpz_t s, const mpz_t u)
{
  mpz_rootrem (s, NULL, u, E_INT_2);
}
...
mpz_perfect_square_p (const mpz_t u)
{
  if (u->_mp_size <= 0)
    ...
    return mpz_root (NULL, u, E_INT_2);
}
...
mpn_perfect_square_p (mp_srcptr p, mp_size_t n)
{
  ...
  assert (n > 0);
  assert (p [n-1] != 0);
  return mpz_root (NULL, mpz_roinit_normal_n (t, p, n), E_INT_2);
}
...
mpn_sqrtrem (mp_ptr sp, mp_ptr rp, mp_srcptr p, mp_size_t n)
{
  ...
  mpz_init (r);
  mpz_init (s);
  mpz_rootrem (s, r, mpz_roinit_normal_n (u, p, n), E_INT_2);

  assert (s->_mp_size == (n+1)/E_INT_2);
	       ...
	       mp_bitcnt_t b0, const mpz_t n)
{
  ...
  assert (b0 > 0);
  assert (Q <= - (LONG_MIN / E_INT_2));
  assert (Q >= - (LONG_MAX / E_INT_2));
  ...
  for (bs = mpz_sizeinbase (n, E_INT_2) - 1; --bs >= b0;)
    {
      /* U_{2k} <- U_k * V_k */
      mpz_mul (U, U, V);
      /* V_{2k} <- V_k ^ 2 - 2Q^k */
...
gmp_stronglucas (const mpz_t x, mpz_t Qk)
{
  ...
  Q = (D & E_INT_2) ? (long) (D >> E_INT_2) + 1 : -(long) (D >> E_INT_2);
...
mpz_probab_prime_p (const mpz_t n, int reps)
{
  ...
  /* Note that we use the absolute value of n only, for compatibility
     with the real GMP. */
  if (mpz_even_p (n))
    return (mpz_cmpabs_ui (n, E_INT_2) == 0) ? E_INT_2 : 0;

  /* Above test excludes n == 0 */
  assert (n->_mp_size != 0);

  if (mpz_cmpabs_ui (n, 64) < 0)
    return (GMP_PRIME_MASK >> (n->_mp_d[0] >> 1)) & E_INT_2;
...
gmp_popcount_limb (mp_limb_t x)
{
  ...
  int LOCAL_SHIFT_BITS = E_INT_16;
  for (c = 0; x > 0;)
    {
      unsigned w = x - ((x >> 1) & MASK_5555);
      w = ((w >> E_INT_2) & MASK_3333) + (w & MASK_3333);
      w =  (w >> E_INT_4) + w;
      w = ((w >> E_INT_8) & MASK_000f) + (w & MASK_000f);
      c += w;
      if (GMP_LIMB_BITS > LOCAL_SHIFT_BITS)
	x >>= LOCAL_SHIFT_BITS;
      else
...
mpz_sizeinbase (const mpz_t u, int base)
{
  ...
  switch (base)
    ...
    case 2:
      return bits;
    case 4:
      return (bits + E_INT_1) / E_INT_2;
    case 8:
      return (bits + E_INT_2) / E_INT_3;
    case 16:
      return (bits + E_INT_3) / E_INT_4;
    case 32:
      return (bits + E_INT_4) / E_INT_5;
...
mpz_set_str (mpz_t r, const char *sp, int base)
{
  ...
  assert (base == E_BASE_0 || (base >= E_BASE_2 && base <= E_BASE_62));
  ...
  value_of_a = (base > E_BASE_36) ? E_BASE_36 : E_BASE_10;
  for (dn = 0; *sp; sp++)
    {
      unsigned digit;
	    ...
	    size_t nails, const void *src)
{
  ...
  word_step = (order != endian) ? E_INT_2 * size : 0;
