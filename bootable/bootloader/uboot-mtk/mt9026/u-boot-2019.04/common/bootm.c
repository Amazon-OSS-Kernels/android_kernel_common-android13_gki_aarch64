			 ...
			 char * const argv[])
{
	...
	case IMAGE_FORMAT_ANDROID:
		...
		char key[BOOTARGS_KEY_LEN]={0},set_cfg[BOOTARGS_CFG_LEN]={0};
		int ret;
		...
		if(mtk_get_vendor_img_hdr_info_u32("h",&version) != 0)
			puts("version info get failure!\n");

		if(version >= ANDROID_HEADER_VERSION_V3){
			if(mtk_get_vendor_img_hdr_info_u64("a",&vendor_addr) != 0){
				puts("Can not get vendor image addr, Android header v3 image can not bring up!\n");
				return 1;
			}

			images.os.load = mtk_android_vendor_image_get_kload((void *)map_sysmem(vendor_addr, 0));
		}else
#endif
			images.os.load = android_image_get_kload(os_hdr);
		images.ep = images.os.load;
		ep_found = true;
		strncpy(key,"androidboot.dtb_idx",sizeof(key)-1);
		ret = snprintf(set_cfg, sizeof(set_cfg)-1,"%s=%d",key,0);
		if(ret < strlen(key))
			debug("dtb_idx is not complete.\n");
...
# endif
	 ...
	 */
    //MTK usb boot
    if(EN_BOOT_DEVICE_USB!=get_boot_device())
    {
	    usb_stop();
    }
#endif
		    ...
		    int states, bootm_headers_t *images, int boot_progress)
{
	...
	u64 start;
	char cmd_s[CMD_BUF_SIZE] = {0};

	start = get_timer(0);
	UBOOT_BOOTTIME("[%s][start:%llu]\n", __func__, start);
	...
	snprintf(cmd_s, CMD_BUF_SIZE - 1, "fdt addr 0x%p", images->ft_addr);
	run_command(cmd_s, 0);
	...
	case IMAGE_FORMAT_ANDROID:
		...
		if(version >= ANDROID_HEADER_VERSION_V3){
			if(mtk_get_vendor_img_hdr_info_u64("a",&vendor_addr) != 0){
				...
				if (mtk_android_image_get_kernel(buf, map_sysmem(vendor_addr, 0), images->verify,
					     os_data, os_len))
					return NULL;
			}
		}else{
