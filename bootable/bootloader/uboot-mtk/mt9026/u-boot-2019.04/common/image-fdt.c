	...
	if (select || genimg_has_config(images)) {
		...
		if(version >= ANDROID_HEADER_VERSION_V3){
			if(mtk_get_vendor_img_hdr_info_u64("a",&vendor_addr) != 0){
				...
				if(!mtk_android_image_get_dtb(map_sysmem(vendor_addr, 0), &fdt_data, &fdt_len) &&
					!fdt_check_header((char *)fdt_data)) {
					fdt_blob = (char *)fdt_data;
					if (fdt_totalsize(fdt_blob) != fdt_len)
						goto error;

					debug("## Using FDT in Android vendor image area\n");
				} else {
		       ...
		       int of_size, struct lmb *lmb)
{
	...
	int ret = -EPERM;
	int fdt_ret;
	//MTK Fix add max bootargs size for bootargs add to fdt issue
	fdt_increase_size(blob, CONFIG_SYS_BARGSIZE);
	...
	if (lmb)
        ...
        ret = fdt_shrink_to_minimum(blob, FDT_BOOTARGAS_EXTRA_SIZE);
	if (ret < 0)
		goto err;
	of_size = ret;
