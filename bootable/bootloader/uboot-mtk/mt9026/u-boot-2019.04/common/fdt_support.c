...
#define KASLR_SEED_SIZE 8
	unsigned char seed[KASLR_SEED_SIZE];
	...
	memset(seed, 0, KASLR_SEED_SIZE);
	if (generate_random_number(&seed[0], KASLR_SEED_SIZE) < 0)
		printf("Generate random seed failed\n");

	err = fdt_setprop(fdt, nodeoffset, "kaslr-seed", seed, KASLR_SEED_SIZE);
	if (err < 0)
		printf("WARNING: fail to set kaslr-seed %s.\n", fdt_strerror(err));

	return fdt_fixup_stdout(fdt, nodeoffset);
}

const char *fdt_getprop_arch(void *fdt, char *target, int *prop_len)
{
#define ARCH_PROPERTY_NAME "Processor"
	int   nodeoffset;
	int   err,len;
	const char *prop;
	...
	nodeoffset = fdt_path_offset(fdt, target);
	if (nodeoffset < 0){
		return NULL;
	}

	prop = fdt_getprop(fdt, nodeoffset, ARCH_PROPERTY_NAME, &len);
	if (len < 0) {
		return NULL;
	}
	*prop_len = len;
