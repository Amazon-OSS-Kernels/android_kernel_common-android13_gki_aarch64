...
static int reserve_fdt(void)
{
	...
	if (gd->fdt_blob) {
		...
		gd->start_addr_sp = ALIGN(gd->start_addr_sp, MMU_SECTION_SIZE) - MMU_SECTION_SIZE;
		gd->start_addr_sp -= ALIGN(CONFIG_SYS_NONCACHED_MEMORY, MMU_SECTION_SIZE);
		debug("Reserving %lu Bytes for noncache at: %08lx\n",
		      (unsigned long)ALIGN(CONFIG_SYS_NONCACHED_MEMORY, MMU_SECTION_SIZE), gd->start_addr_sp);
		set_nc_start_address(gd->start_addr_sp);
		init_mem_map();
#endif
...
static int reloc_fdt(void)
{
	...
	if (gd->new_fdt) {
#ifdef CONFIG_DTB_OVERLAY_FOR_UBOOT_DTB
		fdt_open_into(gd->fdt_blob, gd->new_fdt, gd->fdt_size);
#else
