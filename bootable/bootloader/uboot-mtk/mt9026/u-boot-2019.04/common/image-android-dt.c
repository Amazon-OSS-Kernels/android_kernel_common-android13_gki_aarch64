...
#define BOOTARGS_DTB_IDX_KEY_LEN 20
#define DTBO_HEADER_SIZE 32
#define DTBO_HEADER_PRE_LINE 16
/**
...
bool android_dt_check_header(ulong hdr_addr)
{
	...
	target = simple_strtoul(env_get("dtboaddr"), 0, DTBO_HEADER_PRE_LINE);
	if (magic != DT_TABLE_MAGIC && target == hdr_addr) {
		printf("hdr_addr:%lx hdr:%p magic:%x DT_TABLE_MAGIC:%x\n", hdr_addr, hdr, magic, DT_TABLE_MAGIC);
		val = (char *)hdr;
		for (i = 0; i < DTBO_HEADER_SIZE; i++) {
			if ((i % DTBO_HEADER_PRE_LINE) == 0)
				printf("\n");
