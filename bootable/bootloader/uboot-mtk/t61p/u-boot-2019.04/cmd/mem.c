...
#ifdef CONFIG_MTK_MIUP
#define MIUP_CMD_VAR_N 5
#define CID_LEN_IDX 5
static int do_memprotect(cmd_tbl_t *cmdtp,
			 int flag, int argc,
			 char * const argv[])
{
	...
	u32 idx;
	u32 max_cid_number = 0;
	struct udevice *devp;

	if (uclass_find_first_device(UCLASS_MIUP, &devp))
		return -ENXIO;

	max_cid_number =
		dev_read_u32_default(devp, "mediatek,max_cid_number", 0);

	if (argc < (MIUP_CMD_VAR_N + 1)) {
		ret = CMD_RET_FAILURE;
		goto RET_FUN;
	}

	miup.cid_len = simple_strtol(argv[CID_LEN_IDX], NULL, 10);
	if (argc < (MIUP_CMD_VAR_N + 1 + miup.cid_len)) {
		ret = CMD_RET_FAILURE;
		goto RET_FUN;
	}
	if (miup.cid_len > max_cid_number)
		return -EINVAL;

	miup.prot_blk = simple_strtol(argv[1], NULL, 10);
	miup.attr = simple_strtol(argv[2], NULL, 10);
	miup.start_addr = simple_strtol(argv[3], NULL, 16);
	miup.addr_len = simple_strtol(argv[4], NULL, 16);

	miup.cid = kmalloc((sizeof(u64) * miup.cid_len), GFP_KERNEL);

	for (idx = 0; idx < miup.cid_len; idx++)
		miup.cid[idx] =
		simple_strtol(argv[(idx + MIUP_CMD_VAR_N + 1)], NULL, 16);

	ret = mtk_miup_set_protect(miup);
...
#ifdef CONFIG_MTK_MIUP
U_BOOT_CMD(miup, 24, 0, do_memprotect,
	   "Do memory protect",
	   "- do memory protect in MIUP device"
);
#endif
