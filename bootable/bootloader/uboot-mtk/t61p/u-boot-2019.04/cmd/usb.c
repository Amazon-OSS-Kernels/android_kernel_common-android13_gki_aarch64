...
#ifdef CONFIG_USB_HOST_ETHER
#ifdef CONFIG_DM_ETH
static int find_usb_ether(void)
{
	struct udevice *it;
	struct uclass *uc;
	...
	uclass_foreach_dev(it, uc) {
		if (it->parent && it->parent->driver->id == UCLASS_USB_HUB) {
			printf("%s: Found an ETH USB device: %s\n", __func__, it->name);
			env_set("ethact", it->name);
			return 0;
		}
	}
...
static int find_usb_ether(void)
{
	return usb_ether_curr_dev;
}
#endif

int fastboot_do_usb_start(void)
{
	int ret = 0;
	...
	/* Retry once for some dongles */
	usb_stop();
	do_usb_start();

	return find_usb_ether();
}
#endif
#ifdef CONFIG_DM_USB
