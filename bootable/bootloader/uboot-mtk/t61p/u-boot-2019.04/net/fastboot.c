...
/* If IP defragmentation feature is enabled, we can increase the packet
 * size above the standard limitation of 1512-bytes due to MTU.
 */
...
/**
 * fastboot_timed_send_info() - Send INFO packet every 3 seconds
 *
 * @msg: String describing the reason for waiting
 *
...
static void fastboot_timed_send_info(const char *msg)
{
	...
	/* Send INFO packet to host every 3 seconds */
	if (time >= 3000) {
		start = get_timer(0);
