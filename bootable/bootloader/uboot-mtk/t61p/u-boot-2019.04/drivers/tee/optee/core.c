...
void *optee_alloc_and_init_page_list(void *buf, ulong len, u64 *phys_buf_ptr)
{
	...
	list_size = DIV_ROUND_UP(num_pages, PAGELIST_ENTRIES_PER_PAGE) *
		    page_size;
	page_list = memalign_TEE(page_size, list_size);
	if (!page_list)
		return NULL;
 ...
 *	error code.
 */
static int tee_shm_get_pa(struct tee_shm *shm, size_t offs, phys_addr_t *pa)
{
	if (offs >= shm->size)
		return -EINVAL;
	if (pa)
		*pa = ((uintptr_t)shm->addr + offs);
	return 0;
}
#endif

static int teec_pre_process_tmpref(struct udevice *dev,
...
#ifdef MTK_STATIC_SHM
			rc = __tee_shm_add(dev, OPTEE_MSG_NONCONTIG_PAGE_SIZE, (void *)((uintptr_t)p->u.memref.shm_offs),
			   p->u.memref.size, 0,
			   &shm_buf);
			if (rc)
				return rc;
			mp->u.tmem.shm_ref = (uintptr_t)shm_buf;
#else
			rc = tee_shm_register(dev, (void *)((uintptr_t)p->u.memref.shm_offs),
			   p->u.memref.size, 0,
			   &shm_buf);
			if (rc)
				return rc;
			mp->u.rmem.shm_ref = (uintptr_t)shm_buf;
#endif
			...
			const struct tee_param *params)
{
	...
	for (n = 0; n < num_params; n++) {
		...
		case TEE_PARAM_ATTR_TYPE_MEMREF_INOUT:
			...
			//start of code added by MTK
#ifdef MTK_STATIC_SHM
			//We use OPTEE_MSG_ATTR_TYPE_TMEM* as SHM ref
			mp->attr = OPTEE_MSG_ATTR_TYPE_TMEM_INPUT + p->attr -
				   TEE_PARAM_ATTR_TYPE_MEMREF_INPUT;
			mp->u.tmem.size = p->u.memref.size;
			if (!mp->u.tmem.shm_ref) {
				mp->u.tmem.buf_ptr = 0;
				return 0;
			}
			phys_addr_t pa;
			int rc = -EINVAL;
			rc = tee_shm_get_pa((struct tee_shm *)((uintptr_t)mp->u.tmem.shm_ref), 0, &pa);
			if (rc)
				return rc;
...
#else
			//We use OPTEE_MSG_ATTR_TYPE_RMEM* as SHM ref
			mp->attr = OPTEE_MSG_ATTR_TYPE_RMEM_INPUT + p->attr -
				   TEE_PARAM_ATTR_TYPE_MEMREF_INPUT;
			mp->u.rmem.size = p->u.memref.size;
			mp->u.rmem.offs = 0;
			  ...
			  const struct optee_msg_param *msg_params)
{
	...
	for (n = 0; n < num_params; n++) {
		...
		case OPTEE_MSG_ATTR_TYPE_RMEM_INOUT:
			...
			p->u.memref.shm = shm;
			break;
		//start of code added by MTK, for OPTEE_MSG_ATTR_TYPE_TMEM* cases
		case OPTEE_MSG_ATTR_TYPE_TMEM_INPUT:
		case OPTEE_MSG_ATTR_TYPE_TMEM_OUTPUT:
		case OPTEE_MSG_ATTR_TYPE_TMEM_INOUT:
			//free shm_ref of tmem
			tee_shm_free((struct tee_shm *)((uintptr_t)mp->u.tmem.shm_ref));
			break;
			//end of code added by MTK
		default:
			return -EINVAL;
			     ...
			     uint num_params, struct tee_param *params)
{
	...
	//start of code added by MTK
	rc = teec_pre_process_tmpref(dev, msg_arg->params, num_params, params);
	if (rc) {
		goto out;
	}
	//end of code added by MTK
...
static void print_os_revision(optee_invoke_fn *invoke_fn)
{
	...
	if (res.result.build_id)
		...
		UBOOT_INFO("OP-TEE revision %lu.%lu (%08lx)\n", res.result.major,
		      res.result.minor, res.result.build_id);
	else
		//Modified by MTK
		/*debug("OP-TEE revision %lu.%lu\n", res.result.major,
		      res.result.minor);*/
		UBOOT_INFO("OP-TEE revision %lu.%lu\n", res.result.major,
		      res.result.minor);
}
