...
int usb_tx_comp;
static void usb_tx_complete(struct usb_ep *ep, struct usb_request *req)
{
    usb_tx_comp=1;
}
...
static int fastboot_tx_write_sync(const char *buffer, unsigned int buffer_size)
{
    struct usb_request *in_req = fastboot_func->in_req;
    int ret;
    int count;
    memcpy(in_req->buf, buffer, buffer_size);
    in_req->length = buffer_size;
    in_req->complete= usb_tx_complete;
...
int fastboot_tx_infor(const char *buffer, unsigned int buffer_size)
{
/* All 3 configurations will be enabled if AMZN_FEATURE_UBOOT_USB_HOST_ETHERNET support */
#if defined(CONFIG_USB_HOST_ETHER) && defined(CONFIG_USB_ETHER_ASIX) && defined(CONFIG_USB_ETHER_ASIX88179)
	return -1;
#endif
	return fastboot_tx_write_sync(buffer,buffer_size);
}
