...
void mtk_pnl_set_vby1_mft_hmirror_v006(struct udevice *dev)
{
	...
	printf("[wei]<%ld> %s %d\n", get_timer(0), __FUNCTION__, __LINE__);
}
...
void mtk_out_if_hbkproch_protect_init_v006(struct udevice *dev)
{
	...
	uint8_t u8H_minus_line = 0;

	uint8_t u8Meta_data_mode_shift_line = 0;
	uint16_t u16Hstart_div_lane = 0;
	uint16_t u16Hread_de_st_set = 0;

	W2BYTEMSK(REG_01E4_MODV11, 0x1, REG_01E4_MODV11_REG_HS_WID_ADJ_EN);

	if (priv->lanes >= 8)
		W2BYTEMSK(REG_000C_MODV11, (priv->lanes / 8) - 1,
			  REG_000C_MODV11_REG_MFT_VLD_WRAP_FIX_HTT);
	...
	if ((u16Hstart_div_lane + u8Meta_data_mode_shift_line) >= (u8H_minus_line))
		u16Hread_de_st_set =
		    u16Hstart_div_lane - u8H_minus_line + u8Meta_data_mode_shift_line;
	else
	...
	if ((priv->hsync_w / priv->lanes) == 0)
		...
		W2BYTEMSK(REG_0014_MODV11, (priv->hsync_w / priv->lanes) - 1,
			  REG_0014_MODV11_REG_HS_FP_DELAY_VALUE);
