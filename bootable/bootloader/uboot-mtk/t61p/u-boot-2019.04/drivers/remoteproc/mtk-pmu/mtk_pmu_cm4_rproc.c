...
#define PMU_ADDR_NUM			(4)
#define PMU_ADDR_SHIFT			(32)
#define PMU_ADDR_IDX0			(0)
#define PMU_ADDR_IDX1			(1)
#define PMU_ADDR_IDX2			(2)
#define PMU_ADDR_IDX3			(3)
...
/**
 * struct mtk_pmu_privdata - Structure representing System Controller data.
 * @chan_tx:    Transmit mailbox channel
 * @chan_rx:    Receive mailbox channel
 * @desc:    SoC description for this instance
 * @seq_nr:    Counter for number of messages sent.
 */
struct mtk_pmu_privdata {
...
/**
 * mtk_pmu_init() - Initialize the remote processor
 * @dev:    rproc device pointer
 *
 * Return: 0 if all went ok, else return appropriate error
 */
static int mtk_pmu_init(struct udevice *dev)
{
	mtk_pmu_info info;
	struct mtk_pmu_privdata *priv = dev_get_priv(dev);
	void __iomem *base;
	/* Enable the module */

#define RPMSG_SHM_SIZE (0x15000) // patch : need to refactor
	base = ioremap(priv->shm.start, RPMSG_SHM_SIZE);
	if (!base) {
		PMU_log(PMU_ERROR, "ioremap() fail.\n");
		return -ENOMEM;
	}
	memset(base, 0, RPMSG_SHM_SIZE);
	flush_cache((ulong)base, RPMSG_SHM_SIZE);
	iounmap(base);
#undef RPMSG_SHM_SIZE
...
/**
 * mtk_pmu_load() - Loadup the cm4 remote processor
 ...
 */
static int mtk_pmu_load(struct udevice *dev, ulong addr, ulong size)
{
    struct mtk_pmu_privdata *priv = dev_get_priv(dev);
    int ret = 0, i = 0;
	...
	if (total_size > priv->fw_size) {
		PMU_log(PMU_ERROR, "Overflow, total_size(%x) > priv->fw_size(%016llX)\n",
...
/**
 * mtk_pmu_start() - Start the remote processor
 ...
 * Return: 0 if all went ok, else return appropriate error
 */
static int mtk_pmu_start(struct udevice *dev)
{
	int ret = 0;
	mtk_pmu_info info;
	struct mtk_pmu_privdata *priv = dev_get_priv(dev);
...
/**
 * mtk_pmu_stop() - Stop the remote processor
 *        Note that while technically the cm4 system controller starts up
 *        automatically after its firmware got loaded we still want to
 *        utilize the rproc start operation for other startup-related
 *        tasks.
 ...
 * Return: 0 if all went ok, else return appropriate error
 */
static int mtk_pmu_stop(struct udevice *dev)
{
	int ret = 0;

	PMU_log(PMU_DEBUG, "%s: Ignore stop notification on dev %s\n",
		__func__, dev->name);

	return ret;
}

void * mtk_pmu_device_to_virt(struct udevice *dev, ulong da)
{
	struct mtk_pmu_privdata *priv = dev_get_priv(dev);
	ulong paddr = 0;
 ...
 */
static int mtk_pmu_is_running(struct udevice *dev)
{
	/* TODO: add ipc to notify co-processor. */
	mdelay(2000); // keep timing by CL: 6706979
	return 0;
}

static const struct dm_rproc_ops mtk_pmu_ops = {
	.init = mtk_pmu_init,
	.load = mtk_pmu_load,
	.start = mtk_pmu_start,
	.stop = mtk_pmu_stop,
	.device_to_virt = mtk_pmu_device_to_virt,
	.is_running = mtk_pmu_is_running,
};
...
static int mtk_of_to_priv(struct udevice *dev,
             struct mtk_pmu_privdata *priv)
{
	u32 base = 0;
	u32 addr[PMU_ADDR_NUM] = {0};
	ofnode node;
	...
	if ((!ofnode_valid(node)) ||
	    (ofnode_read_u32_array(node, "reg", addr, PMU_ADDR_NUM) < 0)) {
		PMU_log(PMU_ERROR, "[%s] node(/memory_info/MI_CM4_VOICE/reg) not found.\n",
			__func__);
		return -EIO;
	}

	priv->fw_addr = (((u64)addr[PMU_ADDR_IDX0] << PMU_ADDR_SHIFT) | addr[PMU_ADDR_IDX1])
			- priv->cpu_base;
	priv->fw_size = (((u64)addr[PMU_ADDR_IDX2] << PMU_ADDR_SHIFT) | addr[PMU_ADDR_IDX3]);

	/* Check device. */
	node = ofnode_path("/mmap_info/MI_PMU_RPMSG_VRING0_MEM");
	if (!ofnode_valid(node)) {
		PMU_log(PMU_ERROR, "/mmap_info/MI_PMU_RPMSG_VRING0_MEM not found.\n");
		return -EIO;
	}
	...
	PMU_log(PMU_INFO, "%s(priv->reg.pm_imi=0x%016llX)\n", __func__, (unsigned long long)(priv->reg.pm_imi));
...
/**
 * mtk_pmu_probe() - Basic probe
 * @dev:    corresponding mtk remote processor device
 *
 * Return: 0 if all goes good, else appropriate error message.
 */
static int mtk_pmu_probe(struct udevice *dev)
{
    struct mtk_pmu_privdata *priv = NULL;
    int ret = 0;
...
static const struct udevice_id mtk_pmu_ids[] = {
    {.compatible = "mediatek,merak-pmu-cm4-rproc"},
    {}
};

U_BOOT_DRIVER(mtk_pmu) = {
    .name = "mtk_pmu_cm4_rproc",
    .of_match = mtk_pmu_ids,
    .id = UCLASS_REMOTEPROC,
    .ops = &mtk_pmu_ops,
    .probe = mtk_pmu_probe,
    .priv_auto_alloc_size = sizeof(struct mtk_pmu_privdata),
};
