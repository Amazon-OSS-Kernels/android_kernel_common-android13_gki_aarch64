	...
	if (mmc->erase_feature_support & SEC_GB_CL_EN) {
		...
		if (start_rem) {
			...
			for (blk = start; blk < start + start_rem; blk++)
				mmc_bwrite_without_lock(dev, blk, 1, buf);
...
#if CONFIG_IS_ENABLED(BLK)
ulong mmc_bwrite_without_lock(struct udevice *dev, lbaint_t start, lbaint_t blkcnt, const void *src)
#else
ulong mmc_bwrite_without_lock(struct blk_desc *block_dev, lbaint_t start, lbaint_t blkcnt, const void *src)
#endif
{
#if CONFIG_IS_ENABLED(BLK)
	struct blk_desc *block_dev = dev_get_uclass_platdata(dev);
