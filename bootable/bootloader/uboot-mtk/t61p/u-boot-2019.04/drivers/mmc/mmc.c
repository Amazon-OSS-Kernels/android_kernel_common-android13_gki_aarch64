...
DEFINE_CACHE_ALIGN_BUFFER(u8, excsd_backup, MMC_MAX_BLOCK_LEN);
...
#endif
{
	...
	#if 1
	if ((unsigned long)dst & (ARCH_DMA_MINALIGN-1)) {               
		cur =1;                                                       
		if (mmc_read_blocks(mmc, mmc->sectorbuf, start, cur) != cur) {
		...
		memcpy(dst, mmc->sectorbuf, mmc->read_bl_len);                
		dst += mmc->read_bl_len;                                      
		start++;                                                      
		blocks_todo--;                                                
		if (blocks_todo > 0) {                                         
			IfNotCacheLineAligned = 1;                                  
			blocks_todo--;                                              
		}                                                             
	}                                                               
...
#if !CONFIG_IS_ENABLED(MMC_TINY)
#if !CONFIG_IS_ENABLED(DM_MMC)
static int mmc_select_drive_strength(struct udevice *dev, int mode)
{
	return 0;
}
#endif
			      ...
			      bool hsdowngrade)
{
	int err;
	int speed_bits;
	int driving_strength = 0;
	//ALLOC_CACHE_ALIGN_BUFFER(u8, test_csd, MMC_MAX_BLOCK_LEN);
	u8 *test_csd = excsd_backup;
	...
	case MMC_HS_400:
		...
		driving_strength = mmc_select_drive_strength(mmc, mode);
		break;
#endif
	...
	err = __mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_HS_TIMING,
				speed_bits|(driving_strength << EXT_CSD_DRV_STR_SHIFT), !hsdowngrade);
	if (err)
		return err;
...
static int mmc_startup_v4(struct mmc *mmc)
{
...
#endif
	mmc->erase_feature_support = ext_csd[EXT_SECURE_FEATURE_SUPPORT];
#if CONFIG_IS_ENABLED(MMC_HW_PARTITIONING)
	...
	err = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
		EXT_CSD_POWER_OFF_NOTIFICATION, EXT_CSD_PWR_ON);
	if (err)
		goto error;
#endif // End of defined(MTK_CUSTOMIZATION) && MTK_CUSTOMIZATION
...
#if defined(MTK_CUSTOMIZATION) && MTK_CUSTOMIZATION
int emmc_slc_input_check(struct mmc *mmc, u32 *p_size_mult, u8 *p_wr_rel)
{
	u32 size_mult = *p_size_mult;
	u32 max_enh_size_mult = 0;
	u8 wr_rel = *p_wr_rel;

	if (wr_rel > 1) {
		pr_err("eMMC Err: (SLC) WR_REL should be 0 or 1\n");
		return 1; // Incorrect input
	}

	if (!size_mult && !wr_rel) {
		pr_err("eMMC Err: (SLC) SIZE_MULT = 0 & WR_REL = 0\n");
		return 1; //Incorrect input
	}

	if (!size_mult) {
		pr_err("eMMC: Run (SLC)-> WR_REL ...\n");
		return 0;
	}

	max_enh_size_mult =
		((u32)mmc->ext_csd[EXT_CSD_MAX_ENH_SIZE_MULT + 2] << 16) +
		((u32)mmc->ext_csd[EXT_CSD_MAX_ENH_SIZE_MULT + 1] << 8) +
		mmc->ext_csd[EXT_CSD_MAX_ENH_SIZE_MULT];

	// Change ENH UDA for specail input value (0xffffffff)
	if (size_mult == 0xffffffff) {
		size_mult = max_enh_size_mult;
		*p_size_mult = max_enh_size_mult;
	}

	if (size_mult > max_enh_size_mult) {
		pr_err("eMMC Err: (SLC) ENH_SIZE_MULT > MAX_ENH_SIZE_MULT (%u > %u)\n",
		       size_mult, max_enh_size_mult);
		return 1; // Incorrect input
	}

	// Change wr_rel = 1 when ENH UDA > 0
	if (!wr_rel) {
		pr_err("eMMC: Run (SLC)-> ENH UDA (%u/%u) & WR_REL(forced) ...\n",
		       size_mult, max_enh_size_mult);
		*p_wr_rel = 1;
	} else {
		pr_err("eMMC: Run (SLC)-> ENH UDA (%u/%u) & WR_REL ...\n",
		       size_mult, max_enh_size_mult);
	}
...
int emmc_slc_mode(struct mmc *mmc, u32 size_mult, u8 wr_rel)
{
	u32 enh_size_mult = 0;
	int ret, prttimes;

	/****** Check current eMMC version ******/
	if (mmc->ext_csd[EXT_CSD_REV] < 5) {
		pr_err("eMMC Err: (SLC) Version < MMC v4.4 is not supported\n");
		return 1; //Not support
	}

	/****** Check & change & print input value ******/
	if (emmc_slc_input_check(mmc, &size_mult, &wr_rel))
		return -1; // Incorrect input

	/****** Check current eMMC SPEC of ENH UDA & write reliability ******/
	if (size_mult) {
		if ((mmc->ext_csd[EXT_CSD_PARTITIONING_SUPPORT] & 0x3) != 0x3) {
			pr_err("eMMC Err: (SLC) ENH_ATTRIBUTE is not supported!\n");
			return 1; //Not support
		}
	}

	if (wr_rel) {
		if (((mmc->ext_csd[EXT_CSD_WR_REL_SET] & 0x1) != 0x1) &&
		    ((mmc->ext_csd[EXT_CSD_WR_REL_PARAM] & 0x1) != 0x1)) {
			pr_err("eMMC Err: (SLC) WR_DATA_REL_USR is not supported!\n");
			return 1; //Not support
		}
	}

	/****** Check PARTITION_SETTING_COMPLETED ******/
	if ((mmc->ext_csd[EXT_CSD_PARTITION_SETTING] & 0x1) == 0x1) {
		enh_size_mult =
			((u32)mmc->ext_csd[EXT_CSD_ENH_SIZE_MULT + 2] << 16) +
			((u32)mmc->ext_csd[EXT_CSD_ENH_SIZE_MULT + 1] << 8) +
			mmc->ext_csd[EXT_CSD_ENH_SIZE_MULT];

		pr_err("eMMC: (SLC) PARTITION_SETTING_COMPLETED had been set before\n");
		pr_err("eMMC: (SLC) ECSD[156/167/166] = [%Xh/%Xh/%Xh], ENH_SIZE_MULT = %u\n",
		       mmc->ext_csd[EXT_CSD_PARTITIONS_ATTRIBUTE],
		       mmc->ext_csd[EXT_CSD_WR_REL_SET],
		       mmc->ext_csd[EXT_CSD_WR_REL_PARAM], enh_size_mult);

		if ((mmc->ext_csd[EXT_CSD_WR_REL_SET] & 0x1) != 0x1)
			return 2; //Setting had existed but no write reliability
		else
			return 0; //Setting had existed
	}
	...
	if (size_mult) {
		// ENH_START_ADDR = 0
		mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_ENH_START_ADDR, 0);
		mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_ENH_START_ADDR + 1, 0);
		mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_ENH_START_ADDR + 2, 0);
		mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_ENH_START_ADDR + 3, 0);

		// ENH_SIZE_MULT for UDA
		mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_ENH_SIZE_MULT,
			   (u8)(size_mult & 0xff));
		mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_ENH_SIZE_MULT + 1,
			   (u8)((size_mult & 0xff00) >> 8));
		mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_ENH_SIZE_MULT + 2,
			   (u8)((size_mult & 0xff0000) >> 16));

		mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL,
			   EXT_CSD_PARTITIONS_ATTRIBUTE, 0x1);
	}

	if (wr_rel) {
		if ((mmc->ext_csd[EXT_CSD_WR_REL_SET] & 0x1) == 0x1) {
			pr_err("eMMC: (SLC) WR_DATA_REL_USR had been enabled (default!)\n");

			/**
			 *  Special case: We don't need to set
			 *  PARTITION_SETTING_COMPLETED, because there was no ENH UDA
			 *  setting and WR_DATA_REL_USR = 1 (default)
			 */
			if (!size_mult)
				return 0;
		} else if ((mmc->ext_csd[EXT_CSD_WR_REL_PARAM] & 0x1) == 0x1) {
			mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_WR_REL_SET, 0x1);
		}
		//else case had been processed in input check
	}

	ret = mmc_switch(mmc, EXT_CSD_CMD_SET_NORMAL, EXT_CSD_PARTITION_SETTING, 0x1);

	if (!ret)
		pr_err("eMMC: (SLC) PARTITION_SETTING_COMPLETED is set\n");
	else
		pr_err("eMMC Err: (SLC) PARTITION_SETTING_COMPLETED isn't set (ret = %d)\n",
		       ret);

	prttimes = 300; //Wait 300s
	while (prttimes) {
		printf("\neMMC: (SLC) Please reset the board to active the setting (%d) [%d]\n",
		       ret, prttimes);
		prttimes--;
		mdelay(1000);
	}
...
int emmc_set_pon_off_short(void)
{
