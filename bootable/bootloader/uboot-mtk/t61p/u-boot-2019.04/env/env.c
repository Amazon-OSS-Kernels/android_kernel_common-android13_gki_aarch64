...
int reset_bootargs(void)
{
	char env_cmd_temp[STRING_MAX_LENGTH] = "";

	run_command("env delete bootargs",0);
...
int do_reset_bootargs(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	return reset_bootargs();
}
...
U_BOOT_CMD(
   reset_bootargs ,    CONFIG_SYS_MAXARGS,    1,     do_reset_bootargs,
	"reset_bootargs   - reset bootargs to default\n",
	"    -  \n"
);