...
//add by MTK to support MTK_STATIC_SHM
//#define MTK_STATIC_SHM	1
#define MIU0_BUS_BASE	0x20000000

typedef struct mtk_static_shm_pool {
...
#define malloc_TEE(bytes) mtk_malloc_simple(bytes)
#define memalign_TEE(align, bytes) mtk_memalign_simple(align, bytes)
#define free_TEE(addr)
#else
#define malloc_TEE(bytes) malloc(bytes)
#define memalign_TEE(align, bytes) memalign(align, bytes)
#define free_TEE(addr) free(addr)
#endif
//add by MTK
