...
static struct list_head mmc_part_head = {0};
static struct list_head ufs_part_head = {0};
...
void dev_print (struct blk_desc *dev_desc)
{
	...
	case IF_TYPE_DOC:
		...
		return;
	case IF_TYPE_UFS:
		printf ("(%d) Vendor: %s Prod.: %s Rev: %s\n",
			dev_desc->lun,
			dev_desc->vendor,
			dev_desc->product,
			dev_desc->revision);
...
int part_info_mapping_list_init(struct blk_desc *dev_desc)
{
	if (!dev_desc)
		return -1;

	if (dev_desc->if_type == IF_TYPE_MMC)
		INIT_LIST_HEAD(&mmc_part_head);
	else if (dev_desc->if_type == IF_TYPE_UFS)
		INIT_LIST_HEAD(&ufs_part_head);
	else
		return -1;
...
int part_info_mapping_list_scan(struct blk_desc *dev_desc)
{
	struct part_driver *part_drv;
	struct part_mapping *entry;
	int ret;
	int i;
	disk_partition_t info;
	...
	for (i = 1; i < part_drv->max_entries; i++) {
		...
		entry = malloc(sizeof(struct part_mapping));
		if (!entry)
			return -1;
		entry->idx = i;
		memcpy(&entry->info, &info, sizeof(disk_partition_t));
		debug("Find [%d]Part %s\n", entry->idx, entry->info.name);
		if (dev_desc->if_type == IF_TYPE_MMC)
			list_add_tail(&entry->list, &mmc_part_head);
		else if (dev_desc->if_type == IF_TYPE_UFS)
			list_add_tail(&entry->list, &ufs_part_head);
	}

	return 0;
}

int part_info_search(struct blk_desc *dev_desc, const char *target_name,
		     disk_partition_t *info)
{
	int ret = -1;
	struct part_mapping *pos, *n;

	if (!dev_desc) {
		printf("[ERROR] dev_desc is NULL\n");
		return -1;
	}

	if (strlen(target_name) == 0) {
		printf("[ERROR] target_name is NULL\n");
		return -1;
	}

	if (dev_desc->if_type == IF_TYPE_MMC) {
		ret = list_empty_careful(&mmc_part_head);
		if (ret == 0) {
			list_for_each_entry_safe(pos, n, &mmc_part_head, list) {
				if (strncmp(target_name, (const char *)pos->info.name, strlen(target_name)) == 0 && strlen(target_name) == strlen((const char *)pos->info.name)) {
					memcpy(info, &pos->info, sizeof(disk_partition_t));
					ret = pos->idx;
					return ret;
				}
			}
		...
		ret = list_empty_careful(&ufs_part_head);
		if (ret == 0) {
			list_for_each_entry_safe(pos, n, &ufs_part_head, list) {
				if (strncmp(target_name, (const char *)pos->info.name, strlen(target_name)) == 0 && strlen(target_name) == strlen((const char *)pos->info.name)) {
					memcpy(info, &pos->info, sizeof(disk_partition_t));
					ret = pos->idx;
					return ret;
...
int part_info_del(struct blk_desc *dev_desc)
{
	struct part_mapping *pos, *n;
	int ret;
	...
	if (dev_desc->if_type == IF_TYPE_MMC) {
		...
		if (ret == 0) {
			list_for_each_entry_safe(pos, n, &mmc_part_head, list) {
				debug("Delete [%d]Part %s\n", pos->idx, pos->info.name);
				list_del(&pos->list);
				free(pos);
			}
		}
