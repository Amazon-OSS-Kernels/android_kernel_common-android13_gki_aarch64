...
smp_spin_lock_t mtk_mem_lock = INIT_SPIN_LOCK;
#endif
...
#endif
{
	...
	mchunkptr old_top;
	INTERNAL_SIZE_T old_top_size;
	char *old_end;

	/* Record state of old top */
	old_top     = top;
	old_top_size = chunksize(old_top);
	old_end      = (char *)(chunk_at_offset(old_top, old_top_size));
	...
	if (brk == old_end) { /* can just add bytes to current top */
		...
		if (old_top != initial_top) {
			...
			old_top_size = (old_top_size - SIZE_SZ - SIZE_SZ - SIZE_SZ) & ~MALLOC_ALIGN_MASK;
...
#if defined(CONFIG_MULTICORES_PLATFORM)
	smp_spin_lock_save(&mtk_mem_lock, irq_flag);
#endif
  INTERNAL_SIZE_T nb;

#if CONFIG_VAL(SYS_MALLOC_F_LEN)
	if (!(gd->flags & GD_FLG_FULL_MALLOC_INIT)) {
		temp = malloc_simple(bytes);
#if defined(CONFIG_MULTICORES_PLATFORM)
		smp_spin_unlock_restore(&mtk_mem_lock, irq_flag);
#endif
		return temp;
	}
#endif
