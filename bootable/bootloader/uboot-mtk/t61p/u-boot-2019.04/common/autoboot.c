...
static void bootargs_setting(void)
{
	...
	if(strstr(bootargs,"loglevel") == NULL)
...
#if (defined(TARGET_AMMO_SUPPORT) && defined(UFBL_FEATURE_IDME))
	{
		...
		/* and coyote as AMMO is enabled in the middle of development    */
		if (!(strcmp(ammo_var, ""))) {
			...
			} else if (!(strcmp(board_name, "coyote"))) {
				sprintf(ammo_var, "coyote-ca");
			} else if (!(strcmp(board_name, "coyoteeu"))) {
                                sprintf(ammo_var, "coyoteeu-pv");
