...
#define OLED_VCC_TOGGLE_DELAY (2000)
#define OLED_MONITOR_PERIOD (325)
...
//check VCC, BL, EVDD
    ...
    {
        UBOOT_ERROR("[OLED ERR_DET detect] toggle VCC first\n");
        mtk_panel_enable_vcc(false);
        mdelay(OLED_VCC_TOGGLE_DELAY);  //reference to VCC_OffOn_Delay in dtso
        mtk_panel_enable_vcc(true);

        UBOOT_ERROR("[OLED ERR_DET detect] wait %d ms to make sure\n", OLED_MONITOR_PERIOD);

        //Delay 325ms and check error det gpio again
...
int mtk_set_panel_vcc_cusctrl(MS_BOOL vcc_bl_cusctrl, bool using_tcon_en)
{
    ...
    printf("[mtk_set_panel_vcc_cusctrl] vcc_bl_cusctrl = %d, QHB = %d, tcon en= %d\n",
        vcc_bl_cusctrl, pm_check_back_ground_active(), using_tcon_en);
	...
	if ((pm_check_back_ground_active() == 0) || using_tcon_en)
	{
		...
		if(vcc_bl_cusctrl != 1){
			...
			if (ret != 0)
                ...
                printf("[mtk_set_panel_vcc_cusctrl] VCC_en = 1\n");
		}
	} else {
		if(vcc_bl_cusctrl != 1){
			//get VCC/BL gpio
			...
			if (ret != 0)
                ...
                printf("[mtk_set_panel_vcc_cusctrl] VCC_en = 0\n");
		}
...
int mtk_panel_enable(bool en)
{
    ...
    printf("[mtk_panel_enable] en = %d, bqhq_en = %d, result:%d\n", en, bqhq_en, en|!bqhq_en);
...
int mtk_panel_enable_backlight(bool en)
{
    ...
    if (!ret) {
        ...
        printf("[mtk_panel_enable_backlight] en = %d , BL gpio flag = 0x%lx\n",en, gpio_backlight.flags);
