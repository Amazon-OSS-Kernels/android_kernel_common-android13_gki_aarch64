...
static EN_BOOT_MODE get_upgrade_tigger_mode(void)
{
    ...
    } else if(check_sdboot_mode()){
            mode = EN_BOOT_MODE_SDBOOTKEYPRESS;
    }
...
#define GPIO_NAME_PARENT "/reset-button-gpios-node"
#define GPIO_NAME "reset-button-gpios"
#define RESET_HOLD_VALUE_MS 5000
#define RESET_HOLD_VALUE_INTER_MS 500
static struct gpio_desc reset_ctrl={};

bool check_reset_key_press(void)
{
    bool ret = false;

    ret = (bool) dm_gpio_get_value(&reset_ctrl);
    UBOOT_DEBUG("get gpio dtbo value = 0x%x\n", ret);
    return ret;
}

void long_press_reset_key_detect(void)
{
    unsigned int pre_time = 0;
    unsigned int cur_time = 0;
    unsigned int interval = 0;
    ...
    UBOOT_TRACE("IN\n");
    node = ofnode_path(GPIO_NAME_PARENT);
    if (!ofnode_valid(node)) {
        UBOOT_DEBUG("cannot get reset dts\n");
        goto out;
    }
    gpio_request_by_name_nodev(node, GPIO_NAME, 0, &reset_ctrl, GPIOD_IS_IN);
    if (!dm_gpio_is_valid(&reset_ctrl)) {
        UBOOT_DEBUG("dm_gpio_is_valid = false\n");
        goto out;
    }
    if(check_reset_key_press() == true) {
        ...
        while (interval < RESET_HOLD_VALUE_MS) {
            if (check_reset_key_press() != true) {
                ...
                dm_gpio_free(reset_ctrl.dev, &reset_ctrl);
                goto out;
            }
            cur_time = get_timer(0);
            interval = cur_time - pre_time;
...
void print_bootreason(int boot_reason)
{
    ...
    case PM_BR_REBOOT_OVER_TEMP:
        ...
        break;
    case PM_BR_RECOVERY_QUIESCENT:
        PRT_BOOTREASON(PM_BR_RECOVERY_QUIESCENT);
        break;
...
int upgrade_mode_check(void)
{
    ...
    else if (boot_mode == EN_BOOT_MODE_POWERKEYLONGPRESS || boot_mode == EN_BOOT_MODE_SDBOOTKEYPRESS)
    {
        uboot_boot_mode = EN_BOOT_MODE_USB_UPGRADE;
...
void jump_kernel_flow_status_check(void)
{
    if (mtk_is_panel_oled_locked() == true)
    {
        printf("%s %d Oled panel lock, into console loop\n", __func__, __LINE__);
        cli_loop();
    }
}
