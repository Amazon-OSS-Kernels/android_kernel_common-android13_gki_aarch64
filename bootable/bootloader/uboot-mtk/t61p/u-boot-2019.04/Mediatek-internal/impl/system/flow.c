...
int do_basic_command_register(void)
{
    ...
    if (!led_get_info(&info) && info != NULL) {
        if (info->led_use_setting && info->led_setting == 0) {
            UBOOT_INFO("led_ui_setting is %d, don't set LED state.\n", info->led_setting);
        }
        else {
            pbuf = &(info->led_command[0]);
            while ((cmd = strsep(&pbuf, ";"))) {
                mtk_add_command_table(cmd, 0, AFTER_CONSOLE_INPUT);
            }
        }
    }
    else {
...
{
    mtk_add_command_table("oled_panel_err_cnt_clear", 0, BEFORE_CONSOLE_INPUT);
