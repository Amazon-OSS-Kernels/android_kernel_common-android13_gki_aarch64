...
static int pm_ini_ps_probe(struct pm_ini_device *priv)
{
	ofnode node = {0};
	struct pm_ini_ps ini = {0};
	u32 get_val = 0;
	u16 ps_gpio = 0;
	int ret = 0;
	const char *status;

	node = ofnode_find_subnode(priv->ini_node, "ps-gpio");
	if (!ofnode_valid(node)) {
		UBOOT_ERROR("[PS_GPIO] ofnode_path(ps_gpio) not found.\n");
		return -EINVAL;
	}
	...
	/* only skip if status existed with disable string */
	if ((status) && (strncmp(status, "disabled", DTS_STAT_DISABLE_NUM) == 0)) {
		UBOOT_INFO("[PS_GPIO] skip ps-gpio\n");
		return 0;
	}

	ret = ofnode_read_u32(node, "wakeup-gpio-num", &get_val);
	if (ret) {
		UBOOT_ERROR("[PS_GPIO] read (wakeup-gpio-num) failed.(%d)\n", ret);
		return -EINVAL;
	}
	ps_gpio = (u16)get_val;

	UBOOT_INFO("[PS_GPIO] from dts ps_gpio = 0x%x\n", ps_gpio);


	/* handle endian for pm51 here */
	ini.ps_gpio = cpu_to_be16(ps_gpio);


	UBOOT_INFO("[ps_gpio] to pm51 ps_gpio = 0x%x\n", ini.ps_gpio);


	memcpy((void *)priv->buff_addr + PM_OFS_PS, (void *)&ini, sizeof(ini));
