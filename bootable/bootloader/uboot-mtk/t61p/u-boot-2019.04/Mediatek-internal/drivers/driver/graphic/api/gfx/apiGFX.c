...
typedef void (*FPGFX_CBGetFont)(MS_S8, GFX_FontInfo*);
static FPGFX_CBGetFont fpGFX_CBGetFont = NULL;
typedef void (*FPGFX_CBGetBMP)(MS_S16, GFX_BitmapInfo*);
static FPGFX_CBGetBMP fpGFX_CBGetBMP = NULL;

#ifdef MSOS_TYPE_LINUX
static pthread_mutex_t          GFXFireInfoMutex = PTHREAD_MUTEX_INITIALIZER;
#endif
GFX_FireInfo                    GFXFireInfo;
GFX_Set_VQ                      GFXSerVQ = {0,0,0,0x4,0x4};
//----------------------------------------------------------------------------
// Local Defines
//----------------------------------------------------------------------------
//----------------------------------------------------------------------------
...
#ifndef CONFIG_GFX_REDUCE_CODESIZE_FOR_MBOOT
...
#ifdef CONFIG_GFX_UTOPIA10
        if(Ioctl_GFX_CB(NULL,(void*)&GFXCBInfo) != UTOPIA_STATUS_SUCCESS)
        {
...
GFX_Result MApi_GFX_RegisterGetFontCB(MS_PHY (*fpGFX_CB)(MS_S8, GFX_FontInfo*)  )
{
    ...
    fpGFX_CBGetFont = (FPGFX_CBGetFont)fpGFX_CB;
#ifdef GFX_UTOPIA2K
#else
    if(pGEInstance !=NULL)
    {
...
GFX_Result MApi_GFX_RegisterGetBMPCB(MS_PHY (*fpGFX_CB)(MS_S16, GFX_BitmapInfo*)  )
{
    ...
    fpGFX_CBGetBMP = (FPGFX_CBGetBMP)fpGFX_CB;
#ifdef GFX_UTOPIA2K
#else
    if(pGEInstance !=NULL)
    {
...
#ifdef CONFIG_GFX_UTOPIA10
    if(Ioctl_GFX_Font(NULL,(void*)&GFXTextArgs) != UTOPIA_STATUS_SUCCESS)
    {
        GFX_ERR("%s fail [LINE:%d]\n",__FUNCTION__, __LINE__);
        enRet = GFX_FAIL;
    }
...
#ifdef CONFIG_GFX_UTOPIA10
    if(Ioctl_GFX_Bitmap(NULL,(void*)&pGFXBitmap) != UTOPIA_STATUS_SUCCESS)
    {
        GFX_ERR("%s fail [LINE:%d]\n",__FUNCTION__, __LINE__);
        enRet = GFX_FAIL;
    }
...
void MApi_GFX_Init(GFX_Config *geConfig)
{
    ...
    if(s32GEWrapperMutex < 0)
    {
        GFX_ERR("[%s][%d]  MsOS_CreateMutex Fail\n",__func__,__LINE__);
    }*/
...
#ifdef CONFIG_GFX_UTOPIA10
    if(Ioctl_GFX_TLB(NULL,(void*)&GFXSetTLBConfig) != UTOPIA_STATUS_SUCCESS)
    {
        GFX_ERR("%s fail [LINE:%d]\n",__FUNCTION__, __LINE__);
        enRet = GFX_FAIL;
    }
...
#ifdef CONFIG_GFX_UTOPIA10
    if(Ioctl_GFX_TLB(NULL,(void*)&GFXTLBARGS) != UTOPIA_STATUS_SUCCESS)
    {
        GFX_ERR("%s fail [LINE:%d]\n",__FUNCTION__, __LINE__);
        enRet = GFX_FAIL;
    }
...
#ifdef CONFIG_GFX_UTOPIA10
    if(Ioctl_GFX_TwoSrcBitBlt(NULL,(void*)&GFXTwoSrcBitBlt) != UTOPIA_STATUS_SUCCESS)
    {
        GFX_ERR("%s fail [LINE:%d]\n",__FUNCTION__, __LINE__);
        enRet = GFX_FAIL;
    }
...
GFX_Result MApi_GFX_GetInfo( GFX_DbgInfo *dbg )
{
    ...
    APIGFX_CHECK_INIT();
    GFX_GETINFO_ARGS   GFXGetInfo;
    GFX_DbgInfo        *pGFXDbgInfo;

    pGFXDbgInfo = (GFX_DbgInfo *)malloc(sizeof(GFX_DbgInfo));
    if (pGFXDbgInfo == NULL)
    {
        return GFX_FAIL;
    }
    memset(&GFXGetInfo, 0, sizeof(GFX_GETINFO_ARGS));
    memset(pGFXDbgInfo, 0 ,sizeof(GFX_DbgInfo));

    GFXGetInfo.eGFX_GetConfig         =E_GFX_GET_DBGINFO;
    GFXGetInfo.pGFX_GetInfo           =(void*)pGFXDbgInfo;
...
#ifdef CONFIG_GFX_UTOPIA10
    if(Ioctl_GFX_GetConfig(NULL,(void*)&GFXGetConfig) != UTOPIA_STATUS_SUCCESS)
    {
        GFX_ERR("%s fail [LINE:%d]\n",__FUNCTION__, __LINE__);
        enRet = GFX_FAIL;
    }
...
#ifdef CONFIG_GFX_UTOPIA10
    if(Ioctl_GFX_GetCaps(NULL,(void*)&GFXGetCaps) != UTOPIA_STATUS_SUCCESS)
    {
        GFX_ERR("%s fail [LINE:%d]\n",__FUNCTION__, __LINE__);
        enRet = GFX_FAIL;
    }
...
///GFX STR Mode
/******************************************************************************/
MS_U32 MApi_GFX_SetPowerState(EN_GFX_POWER_MODE u16PowerState)
{
...
#ifdef CONFIG_GFX_UTOPIA10
    if(Ioctl_GFX_PowerState(NULL,(void*)&pGFXPowerStateArg) != UTOPIA_STATUS_SUCCESS)
    {
        GFX_ERR("%s fail [LINE:%d]\n",__FUNCTION__, __LINE__);
...
/******************************************************************************/
///GFX set CRC
///@param E_GE_CRC_ENABLE: MS_BOOL
///@param E_GE_CRC_CLEAR: MS_BOOL
/******************************************************************************/
GFX_Result _MApi_GE_ConfigCRC(EN_GE_ConfigCRC enConfigCRC , void* pArgs)
{
    GFX_Result  enRet = GFX_SUCCESS;
    GFX_MISC_ARGS   GFXInfo;
    ST_GFX_ConfigCRC stCRCInfo;

    APIGFX_CHECK_INIT();
    memset(&GFXInfo, 0, sizeof(GFX_MISC_ARGS));
    memset(&stCRCInfo, 0, sizeof(ST_GFX_ConfigCRC));

    stCRCInfo.enConfigCRC = enConfigCRC;
    stCRCInfo.pCRCsetting = pArgs;

    GFXInfo.eGFX_MISCType = E_MISC_SET_CRC;
    GFXInfo.pGFX_Info     = (void*)&stCRCInfo;
    GFXInfo.u32Size       = sizeof(ST_GFX_ConfigCRC);
