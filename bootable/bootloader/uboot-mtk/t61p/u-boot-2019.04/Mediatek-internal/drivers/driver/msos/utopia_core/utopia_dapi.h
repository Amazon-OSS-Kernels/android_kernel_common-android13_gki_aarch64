...
#if defined(ANDROID) && !defined(ANDROID_RECOVERY)
    #include <cutils/log.h>
    #define printu              ALOGI
    #define flagPrintu(FLAG)    if(FLAG)printu
#elif defined(MSOS_TYPE_LINUX_KERNEL)
    #define printu(fmt,...)     printk(KERN_ERR fmt, ##__VA_ARGS__)
    #define flagPrintu(FLAG)    if(FLAG)printu
#else
...
#if defined(MSOS_TYPE_LINUX_KERNEL)
#define ULOGTAG_LEN 16
typedef struct
{
    struct list_head list;
    char tag[ULOGTAG_LEN];
} ULOGTAG_PARAM;
#endif
...
#if defined(MSOS_TYPE_LINUX_KERNEL)
MS_BOOL UtopiaLogSystem(const char* tag);
#endif
