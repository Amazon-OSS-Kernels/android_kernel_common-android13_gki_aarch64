...
typedef enum
{
  E_MIU_CHANNEL_0 = 0x10, // Different from E_MIU_0
  E_MIU_CHANNEL_1,
  E_MIU_CHANNEL_NUM,
  E_MIU_CHANNEL_ALL = 0xFF,
} MIU_Channel;
...
typedef enum
{
  ...
  E_MIU_DDR_4096MB,
  E_MIU_DDR_8192MB,
  E_MIU_DDR_384MB,  //256+128
  E_MIU_DDR_768MB,  //512+256
  E_MIU_DDR_1280MB, //1024+256
  E_MIU_DDR_1536MB, //1024+512
  E_MIU_DDR_1792MB, //1024+768
  E_MIU_DDR_2304MB, //2048+256
  E_MIU_DDR_2560MB, //2048+512
  E_MIU_DDR_2816MB, //2048+768
  E_MIU_DDR_3072MB, //2048+1024
  E_MIU_DDR_3328MB, //2048+1280
  E_MIU_DDR_3584MB, //2048+1536
  E_MIU_DDR_3840MB, //2048+1792
}MIU_DDR_SIZE;
...
typedef struct
{
    ...
    MS_U32 u32ReportImmediateUsage;
    MS_U32 u32ReportMaxUsage;
    MS_U32 u32ReportImmediateBW;
    MS_U32 u32ReportMaxBW;
...
typedef struct DLL_PACKED
{
    ...
    MS_BOOL                         bMonitorAreaEn;                                             //Monitor Area Only
}MIU_BandwidthMonitorInfo;

/// debug level
typedef enum
{
    E_MIU_DBGLV_NONE,          /// no debug message
    E_MIU_DBGLV_ERROR,         /// show error
    E_MIU_DBGLV_DEBUG,         /// show error & warning
    E_MIU_DBGLV_INFO,          /// show error & warning & informaiton
    E_MIU_DBGLV_ALL,           /// show error & warning & information & funciton name
}MIU_DBGLV;

typedef enum
{
    E_BW_TYPE_IMMED_EFFI = 0,    //Immediate Efficiency
    E_BW_TYPE_MIN_EFFI,          //Min Efficieny
    E_BW_TYPE_IMMED_UTIL,        //Immediate utilization
    E_BW_TYPE_MAX_UTIL,          //Max utilization
}MIU_BW_TYPE;
...
typedef struct DLL_PACKED
{
    MS_U32 u32MIU0BWValue;
    MS_U32 u32MIU1BWValue;
    MS_U32 u32MIU2BWValue;
    MS_U32 u32UMATotal;
    MS_U32 u32UMATail;
}MIU_BWValue;
...
typedef enum
{
    ...
    E_MIU_MODULE_VDEC,
    E_MIU_MODULE_GE,
    E_MIU_MODULE_VDEC_R2,
    E_MIU_MODULE_DIP,
    E_MIU_MODULE_VE,
    E_MIU_MODULE_MFDEC,
    E_MIU_MODULE_BIST,
    E_MIU_MODULE_AUDIO_EN,
    E_MIU_MODULE_AUDIO_DE,
    E_MIU_MODULE_DISPLAY,
}MIU_IP_Module;

typedef struct DLL_PACKED
{
    MS_U32  u32MIUDramBus;
    MS_U32  u32MIUDramDataRatio;
    MS_U32  u32MIUDramFreq;
    MS_U32  u32MIUDDRPhaseErrorCheck;
}MIU_Info;
...
#define TAG_MIU "MIU"
#define MIU_DEBUG_INFO(dbgl, x, args...)    if (dbgl >= E_MIU_DBGLV_INFO ) \
                                            {ULOGI(TAG_MIU, "\t[%s][%d]", __FUNCTION__, __LINE__); ULOGI(TAG_MIU, x, ##args);}
#define MIU_DEBUG_DBG(dbgl, x, args...)     if (dbgl >= E_MIU_DBGLV_DEBUG ) \
                                            {ULOGI(TAG_MIU, "\t[%s][%d]", __FUNCTION__, __LINE__); ULOGD(TAG_MIU, x, ##args);}
#define MIU_DEBUG_ERR(dbgl, x, args...)     if (dbgl >= E_MIU_DBGLV_ERROR ) \
                                            {ULOGI(TAG_MIU, "\t[%s][%d]", __FUNCTION__, __LINE__); ULOGE(TAG_MIU, x, ##args);}
#define MIU_DEBUG_PRI(dbgl, x, args...)     {printf(x, ##args);}
...
//pBWInfo size need to be (MIU_MAX_BW_MONITOR_GROUP_NUMBER * MIU_BandwidthMonitorInfo)
MS_BOOL MDrv_MIU_Get_BW_All(MS_U8 u8MiuDev, MIU_BandwidthMonitorInfo *pBWInfo);
//-------------------------------------------------------------------------------------------------
/// @ingroup G_MIU_BW_Monitor
/// @param u8MiuDev \b IN: miu device id
/// @return true : Success
...
MS_BOOL MDrv_MIU_Start_BW_Monitor(MS_U8 u8MiuDev);

MS_BOOL MDrv_MIU_ConfigHWAttr(MS_U8 u8MiuDev, eMIU_ATTR_TYPE eAttrType, MS_U16 *u16Value, MS_U16 u16Length);

MS_BOOL MDrv_MIU_SetDbgLevel(MIU_DBGLV eDBGLV);

MS_BOOL MDrv_MIU_SetClientUtilization(MIU_ID eMIU, eMIUClientID eClientID);

MS_BOOL MDrv_MIU_GetClientUtilization(MIU_ID eMIU, MS_U32 *u32BWUtilization);

MS_BOOL MDrv_MIU_SetClientEfficiency(MIU_ID eMIU, eMIUClientID eClientID);

MS_BOOL MDrv_MIU_GetClientEfficiency(MIU_ID eMIU, MS_U32 *u32BWEfficiency);

MS_BOOL MDrv_MIU_SetBWValue(MS_U32 u32ClientID, MIU_BW_TYPE eBWInfoType);

MS_BOOL MDrv_MIU_GetBWValue(MS_U32 u32ClientID, MIU_BW_TYPE eBWInfoType, MIU_UMA_Mode *eUMAMode, MIU_BWValue *BWValue);

MS_U32 MDrv_MIU_GetModuleClient(MIU_ID eMiu, MIU_IP_Module eMiuModule, MS_U32 **u32ModuleClient);

MS_BOOL MDrv_MIU_GetMIUInfo(MIU_ID eMiu, MIU_Info* stMIUInfo);

MS_BOOL MDrv_MIU_SetCPUHPriority(MIU_HPri_Scope eScope, MS_U64 u64Start, MS_U64 u64End, MS_BOOL bEnable);

MS_BOOL MDrv_MIU_GetSWClientInfo(MIU_ID eMIU, MS_U64 *pu64HWClientID, MS_U32 u32HWClientIDNumber);
