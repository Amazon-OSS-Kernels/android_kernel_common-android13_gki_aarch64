...
#define API_PNLSTATUS_EX_VERSION                         (6)
...
/// ST_PNL_STRINFO version
#define ST_PNL_STRINFO_VERSION                           (4)

/// ST_PNL_VRR_INFO version
#define ST_PNL_VRR_INFO_VERSION                          (1)

/// ST_PNL_MODSTATUS version
#define ST_PNL_MODSTATUS_VERSION                         (1)

// ST_PNL_INI_PARA version
#define ST_PNL_PARA_VERSION                              (1)

/// ST_PNL_CONTROL_OUT_SWING_CHANNEL version
#define ST_PNL_CONTROL_OUT_SWING_CHANNEL_VERSION         (1)

#define ST_PNL_QOS_BW_INFO_VERSION                      (1)
...
typedef enum
{
    ...
    LINK_MINILVDS_4CH_6P_8BIT,///86
    LINK_EPI28_12P_EPI3G,      ///87
    LINK_EPI28_16P_EPI3G,      ///88

    LINK_MIPI_DSI_RGB888_4lane,  ///89
    LINK_MIPI_DSI_RGB565_4lane,  ///90
    LINK_MIPI_DSI_RGB666_4lane,  ///91
    LINK_MIPI_DSI_RGB888_3lane,  ///92
    LINK_MIPI_DSI_RGB565_3lane,  ///93
    LINK_MIPI_DSI_RGB666_3lane,  ///94
    LINK_MIPI_DSI_RGB888_2lane,  ///95
    LINK_MIPI_DSI_RGB565_2lane,  ///96
    LINK_MIPI_DSI_RGB666_2lane,  ///97
    LINK_MIPI_DSI_RGB888_1lane,  ///98
    LINK_MIPI_DSI_RGB565_1lane,  ///99
    LINK_MIPI_DSI_RGB666_1lane,  ///100
...
typedef struct DLL_PACKED
{
    ...
    MS_U16 u16SwingMax;                     ///< Max bond of swing vavlue for api MHal_PNL_MOD_Control_Out_Swing LVDS panel normalization
    MS_U16 u16SwingMin;                     ///< Min bond of swing vavlue for api MHal_PNL_MOD_Control_Out_Swing LVDS panel normalization
    MS_U16 u16SwingCurrent;                 ///< get current swing value

    MS_BOOL bIsPanelDCLKUpdate;             ///Check DClk is update or not
    MS_U32 u32MaxSETfromMaxDCLK;
    MS_U32 u32MinSETfromMinDCLK;
    ...
    MS_U16 u16MaxVTotal;                     // Panel Max VTT
    MS_U16 u16MinVTotal;                     // Panel Min VTT
    MS_BOOL bTCON_VsyncShift;                // TCON Vsync need Shift
    MS_BOOL bSTR_ResumeDone;                 // Panel STR Resume Done Status
} PNL_ApiExtStatus;
...
typedef struct  DLL_PACKED
{
    ...
    MS_U16 u16Swing_Level;         /// Swing value to set
    MS_U16 u16Channel_Select;      /// Swing channel to set
}ST_PNL_CONTROL_OUT_SWING_CHANNEL;

typedef struct DLL_PACKED
{
    MS_U32 u32Version;                      /// Structure version

    MS_U32 u32PanelMaxDCLK;                  ///<  PANEL_MAX_DCLK. Reserved for future using.
    MS_U64 u64PanelDCLK;                     ///<  LPLL_0F[23:0], PANEL_DCLK          ,{0x3100_10[7:0], 0x3100_0F[15:0]}
    MS_U32 u32PanelMinDCLK;                  ///<  PANEL_MIN_DCLK. Reserved for future using.
} ST_PNL_INI_PARA;
...
typedef enum
{
    E_PNL_STR_MODE,  ///STR Mode: Panel STR sequence on mboot
    E_PNL_FSTR_MODE, ///Fast STR Mode: Panel STR sequence on MI/SN and panel drvier
    E_PNL_RFSTR_MODE, ///Refine Fast STR Mode: Panel STR sequence only on panel drvier
    E_PNL_STR_MAX_MAX,
} EN_PNL_STR_MODE;
...
typedef struct DLL_PACKED
{
    MS_U32 u32Version;                 ///Struct version
    MS_U32 u32Length;                  ///Sturct length
    MS_BOOL bGPIO_INV_CTL_INVERTOR;    //GPIO:Backlight
    MS_BOOL bGPIO_PNL_CTL_INVERTOR;    //GPIO:Panel VCC
    MS_U16 u16GPIO_INV_CTL_PAD_NO;
    MS_U16 u16GPIO_PNL_CTL_PAD_NO;
    EN_PNL_STR_MODE eSTRMode;
    PWM_ChNum ePWM_ChNum;    //PWM Channel Number
    MS_BOOL bGPIO_LED_CTL_INVERTOR;    //GPIO:LED invertor
    MS_U16 u16GPIO_LED_CTL_PAD_NO;    //GPIO:LED Pad number
}ST_PNL_STRINFO;
...
typedef enum
{
    E_PNL_MODCRC_R_CHANNEL,
    E_PNL_MODCRC_G_CHANNEL,
    E_PNL_MODCRC_B_CHANNEL,
    E_PNL_MODCRC_CHANNEL_MAX,
}EN_PNL_MODCRC_CHANNEL;

typedef enum
{
    E_PNL_MODCRC_EVEN_PAIR,
    E_PNL_MODCRC_ODD_PAIR,
    E_PNL_MODCRC_PAIR_MAX,
}EN_PNL_MODCRC_PAIR;
...
typedef struct  DLL_PACKED
{
    ...
    EN_PNL_MODCRC_CHANNEL enMODCRCChannel; /// IN: rgb channel select
    EN_PNL_MODCRC_PAIR enMODCRCPair;       /// IN: odd/even select
    MS_U16 *pu16CRCValue;                /// OUT: CRC value
#if !defined (__aarch64__)
    MS_U32 u32AlignDummy0;                     /// Dummy parameter
#endif
}ST_PNL_MODSTATUS;
...
typedef enum
{
    E_PNL_MAIN_WINDOW=0,          ///< main window if with PIP or without PIP
    E_PNL_SUB_WINDOW=1,           ///< sub window if PIP
    E_PNL_MAX_WINDOW              /// The max support window
}EN_PNL_WINDOW;
...
typedef struct  DLL_PACKED
{
    MS_U32 u32Version;                          ///< Sturct verson
    MS_U32 u32Length;                           ///< Sturct length
    EN_PNL_WINDOW enPNLWin;                     ///< Window Id
    EN_PNL_OVERDRIVE_MODE enOverdriveMode;      ///< Overdrive bw level
}ST_PNL_QOS_BW_INFO;
...
//-------------------------------------------------------------------------------------------------
/// Description : Get MOD Status
/// @ingroup PNL_INTERFACE_FEATURE
/// @param  pstGetMODStatus                 \b IN\OUT: pointer of struct for ST_PNL_MODSTATUS
/// @return APIPNL_Result
//-------------------------------------------------------------------------------------------------
APIPNL_Result SYMBOL_WEAK MApi_PNL_GetMODSTATUS(ST_PNL_MODSTATUS *pstGetMODStatus);

//-------------------------------------------------------------------------------------------------
/// Description : Set PNL BW Level
/// @ingroup PNL_INTERFACE_FEATURE
/// @param  ST_PNL_QOS_BW_INFO                 \b IN\OUT: pointer of struct for ST_PNL_QOS_BW_INFO
/// @return APIPNL_Result
APIPNL_Result SYMBOL_WEAK MApi_PNL_SetBWLevel(ST_PNL_QOS_BW_INFO *pstSetBWInfo);
...
MS_BOOL SYMBOL_WEAK MApi_PNL_SetSTRInfo(ST_PNL_STRINFO *pstPNLSTRInfo);
APIPNL_Result SYMBOL_WEAK MApi_PNL_SetVRRInfo(ST_PNL_VRR_INFO *pstVrrInfo);
#undef INTERFACE
