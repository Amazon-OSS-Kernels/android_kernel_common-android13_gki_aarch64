...
#define H_K_C_FORMAT_TYPE_LEN         31
...
static int _mtk_pnl_cust_iic_read(unsigned char  u8SlaveCfg, unsigned long uAddrCnt, unsigned char  *pRegAddr, unsigned long uSize, unsigned char  *pData)
{
    ...
    if(ret != 0)
    {
        UBOOT_DEBUG("dm_i2c_read fail,%d\n",ret);
        return -EPERM;
    }
...
static int _mtk_pnl_cust_ic_auto_p_gamma(st_cust_ic_info *cust_ic, unsigned char *bin_buf, unsigned short size)
{
...
static void _mtk_pnl_cust_ic_read(unsigned char* r_data, int r_size, st_cust_ic_info *cust_info)
{
    uint16_t bus_id, slave_id;
    unsigned long addr_count;
    unsigned char reg_offset;
    ...
    if(ret < 0) {
        UBOOT_DEBUG("read iic error!! bus_id=%d slave_id=0x%02x \n",bus_id, slave_id);
    } else {
...
static int _mtk_pnl_cust_pmic_auto_update_from_flash(st_cust_ic_info *pmic_info)
{
    int retval = -EINVAL;
    ...
    {
        retval = _mtk_pnl_cust_ic_auto_p_gamma(pmic_info, pmic_bin_buf, pmic_size);
        if (retval)
        {
            UBOOT_TRACE("[%d][%d] pmic auto update from flash fail, use default data!\n", __LINE__, retval);
        }
        else
        {
            if ((pmic_bin_buf != NULL) && (pmic_size > 0))
            {
                UBOOT_TRACE("[%d] pmic auto auto update from flash:\n", __LINE__);
                UBOOT_DUMP(pmic_bin_buf, pmic_size);
...
static int _mtk_pnl_cust_pmic_init(st_multi_cust_ic_info *multi_cust_ic)
{
    ...
    if ((pmic_size > 0) && (pmic_bin_buf != NULL))
    {
        ...
        if (pmic_info->bin_format_type == 0)
        {
            _mtk_pnl_cust_ic_read(r_data, pmic_size, pmic_info);
        }
        else if (pmic_info->bin_format_type == 1)
        {
        ...
        UBOOT_TRACE("pmic checksum : %s\n", checkstatus ? "pass": "fail");
        if (checkstatus ==  FALSE)
        {
            UBOOT_TRACE("checksum fail! write agin!\n");
            if (pmic_info->bin_format_type == 0)
            {
                _mtk_pnl_cust_ic_write(pmic_bin_buf, pmic_size, pmic_info);
            }
        ...
        retval = _mtk_pnl_cust_pmic_auto_update_from_flash(pmic_info);
        if (retval == -ENOMEM)
        {
            goto pmic_ic_init_finish;
        }

        if ((pmic_info->with_nvm == 1) && (checkstatus ==  FALSE))
        {
...
static int _mtk_pnl_cust_pgamma_init_auto_update_from_flash(st_cust_ic_info *pgamma_info)
{
    int retval = -EINVAL;
    ...
    {
        retval = _mtk_pnl_cust_ic_auto_p_gamma(pgamma_info, pgamma_bin_buf, pgamma_size);
        if (retval)
        {
        ...
        else
        {
            if ((pgamma_bin_buf != NULL) && (pgamma_size > 0))
            {
                UBOOT_TRACE("[%d] pgamma after auto auto update from flash:\n", __LINE__);
                UBOOT_DUMP(pgamma_bin_buf, pgamma_size);
...
static int _mtk_pnl_cust_pgamma_init(st_multi_cust_ic_info *multi_cust_ic)
{
    ...
    if ((pgamma_size > 0) && (pgamma_bin_buf != NULL))
    {
        ...
        retval = _mtk_pnl_cust_pgamma_init_auto_update_from_flash(pgamma_info);
        if (retval == -ENOMEM)
        {
            goto pgamma_ic_init_finish;
        }

        if ((pgamma_info->with_nvm == 1) && (checkstatus ==  FALSE))
        {
...
static int _mtk_pnl_cust_levelshifit_init_auto_update_from_flash(st_cust_ic_info *ls_info)
{
    int retval = -EINVAL;
    ...
    {
        retval = _mtk_pnl_cust_ic_auto_p_gamma(ls_info, ls_bin_buf, ls_size);
        if (retval)
        {
        ...
        else
        {
            if ((ls_bin_buf != NULL) && (ls_size > 0))
            {
                UBOOT_TRACE("[%d] level shift after auto auto update from flash:\n", __LINE__);
                UBOOT_DUMP(ls_bin_buf, ls_size);
...
static int _mtk_pnl_cust_levelshifit_init(st_multi_cust_ic_info *multi_cust_ic)
{
    ...
    if ((ls_size > 0) && (ls_bin_buf != NULL))
    {
        ...
        UBOOT_TRACE("levelshift ic checksum : %s\n", checkstatus ? "pass": "fail");
        if (checkstatus ==  FALSE)
        {
            UBOOT_TRACE("checksum fail! write agin!\n");
            if (ls_info->bin_format_type == 0)
        ...
        retval = _mtk_pnl_cust_levelshifit_init_auto_update_from_flash(ls_info);
        if (retval == -ENOMEM)
        {
            goto ls_ic_init_finish;
        }

        if ((ls_info->with_nvm == 1) && (checkstatus ==  FALSE))
        {
...
static int _mtk_pnl_cust_vcomic_init_auto_update_from_flash(st_cust_ic_info *vcomic_info)
{
    int retval = -EINVAL;
    ...
    {
        retval = _mtk_pnl_cust_ic_auto_p_gamma(vcomic_info, vcomic_bin_buf, vcomic_size);
        if (retval)
        {
        ...
        else
        {
            if ((vcomic_bin_buf != NULL) && (vcomic_size > 0))
            {
                UBOOT_TRACE("[%d] vcom ic after auto auto update from flash:\n", __LINE__);
                UBOOT_DUMP(vcomic_bin_buf, vcomic_size);
...
static int _mtk_pnl_cust_vcomic_init(st_multi_cust_ic_info *multi_cust_ic)
{
    ...
    if ((vcomic_size > 0) && (vcomic_bin_buf != NULL))
    {
        ...
        UBOOT_TRACE("vcom ic checksum : %s\n", checkstatus ? "pass": "fail");
        if (checkstatus ==  FALSE)
        {
            UBOOT_TRACE("checksum fail! write agin!\n");
            if (vcomic_info->bin_format_type == 0)
        ...
        retval = _mtk_pnl_cust_vcomic_init_auto_update_from_flash(vcomic_info);
        if (retval == -ENOMEM)
        {
            goto vcomic_ic_init_finish;
        }

        if ((vcomic_info->with_nvm == 1) && (checkstatus == FALSE))
        {
...
static int _mtk_pnl_cust_ic_sub_init_auto_update_from_flash(st_cust_ic_info *cust_ic_info, unsigned char* sub_bin_buf, unsigned short sub_size)
{
    int retval = -EINVAL;
    ...
    {
        retval = _mtk_pnl_cust_ic_auto_p_gamma(cust_ic_info, sub_bin_buf, sub_size);
        if (retval)
        {
        ...
        else
        {
            ...
            {
                UBOOT_TRACE("[%d] sub ic=[%d] after auto auto update from flash:\n", __LINE__, cust_ic_info->ic_type);
...
static int _mtk_pnl_cust_ic_sub_init(st_multi_cust_ic_info *multi_cust_ic, en_pnl_cust_ic_type en_type, unsigned char* sub_bin_buf, unsigned short sub_size)
{
    ...
    if ((sub_size > 0) && (sub_bin_buf != NULL))
    {
        ...
        retval = _mtk_pnl_cust_ic_sub_init_auto_update_from_flash(cust_ic_info, sub_bin_buf, sub_size);
        if (retval == -ENOMEM)
        {
            goto sub_cust_ic_init_finish;
        }

        if ((cust_ic_info->with_nvm == 1) && (checkstatus ==  FALSE))
        {
