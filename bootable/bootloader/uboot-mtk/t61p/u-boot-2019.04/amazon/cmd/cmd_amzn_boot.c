...
void trans_screen_factory_customized(char **bg_color, char **disp_txt, char **txt_color, char **graphic_x, char **graphic_y)
{
	...
	if (strcmp(factory_name, "")) {
		if (!strcmp(factory_name, "hisense")) {
			if ((get_board_revision() != PVT) || ((get_board_revision() == PVT) && target_is_production()))
			{
				*bg_color = BG_GREEN;
				*disp_txt = "OK";
				*txt_color = TXT_WHITE;
...
int initr_diag_fos_trans_screen(void)
{
...
#ifdef CONFIG_TRANS_SCREEN_CUSTOMIZED
	trans_screen_factory_customized(&bg_color, &disp_txt, &txt_color, &graphic_x, &graphic_y);
#endif
	...
	memset(buffer, 0 , CMD_BUF);
	snprintf(buffer, CMD_BUF, "  draw_string %s %s %s 0 %s", graphic_x, graphic_y, txt_color, disp_txt);
	printf("%s\n", buffer);
	run_command(buffer, 0);
